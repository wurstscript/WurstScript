...
public class TransitiveClosure<T> {
  ...
  /**
   * returns all elements reachable from start (start is only included if it is reachable with > 0
   * steps)
   */
  ...
  public Stream<T> get(T start) {
    ...
    return base.get(start).stream().flatMap(c -> visit(c, visited));
  }
  ...
  private Stream<T> visit(T start, Set<T> visited) {
    ...
    return Stream.concat(
        Stream.of(start), base.get(start).stream().flatMap(c -> visit(c, visited)));
  }
}
