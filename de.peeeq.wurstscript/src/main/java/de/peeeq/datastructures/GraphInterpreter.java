...
public abstract class GraphInterpreter<T> {
  ...
  private @Nullable TopsortResult<T> topSort_add(
      List<T> result, Set<T> seen, List<T> seenStack, T n) {
   ...
   *
   * <p>See https://en.wikipedia.org/wiki/Path-based_strong_component_algorithm
   */
  public Set<Set<T>> findStronglyConnectedComponents(List<T> nodes) {
    // Stack S contains all the vertices that have not yet been assigned to a strongly connected
    // component, in the order in which the depth-first search reaches the vertices.
    Deque<T> s = new ArrayDeque<>();
    // Stack P contains vertices that have not yet been determined to belong to different strongly
    // connected components from each other
    Deque<T> p = new ArrayDeque<>();
    // It also uses a counter C of the number of vertices reached so far, which it uses to compute
  ...
  private void findStronglyConnectedComponentsRec(
      ...
      Deque<T> p,
      AtomicInteger c,
      Map<T, Integer> preorderNumber,
      Map<T, Integer> component,
      AtomicInteger componentCount) {
    ...
    for (T w : getIncidentNodes(v)) {
      if (!preorderNumber.containsKey(w)) {
        ...
        if (!component.containsKey(w)) {
          // Repeatedly pop vertices from P until the top element of P has a preorder number less
          // than or equal to the preorder number of w.
          while (!p.isEmpty()
    ...
    if (!p.isEmpty() && p.peek() == v) {
      // Pop vertices from S until v has been popped, and assign the popped vertices to a new
      // component.
