...
public class ModuleExpander {
  ...
  public static ModuleInstanciations expandModules(
      ClassOrModule m, Collection<ClassOrModule> visited) {
    ...
    for (ModuleUse moduleUse : m.getModuleUses()) {
      ...
      for (int i = 0; i < numTypeArgs; i++) {
        typeReplacements.add(
            Pair.create(
                usedModule.getTypeParameters().get(i).attrTyp(),
                moduleUse.getTypeArgs().get(i).attrTyp()));
      }

      WPos source = moduleUse.getSource().artificial();
      WPos idSource = moduleUse.getModuleNameId().getSource().artificial();
      ModuleInstanciation mi =
          Ast.ModuleInstanciation(
              source,
              Ast.Modifiers(),
        ...
        mi.getConstructors()
            .add(
                Ast.ConstructorDef(
                    ...
                    Ast.WStatements(
                        Ast.StartFunctionStatement(source), Ast.EndFunctionStatement(source))));
      }
  ...
  public static <T extends Element> T smartCopy(
      T e, List<Pair<WurstType, WurstType>> typeReplacements) {
    List<Pair<ImmutableList<Integer>, TypeExpr>> replacementsByPath = Lists.newArrayList();
    calcReplacementsByPath(
        typeReplacements, replacementsByPath, e, ImmutableList.<Integer>emptyList());
  ...
  private static void calcReplacementsByPath(
      List<Pair<WurstType, WurstType>> typeReplacements,
      List<Pair<ImmutableList<Integer>, TypeExpr>> replacementsByPath,
      Element e,
      ImmutableList<Integer> pos) {
    if (e instanceof TypeExpr) {
      ...
      for (Pair<WurstType, WurstType> rep : typeReplacements) {
        if (typeExpr.attrTyp().equalsType(rep.getA(), e)) {
          ...
          replacementsByPath.add(
              Pair.create(pos, (TypeExpr) Ast.TypeExprResolved(source, rep.getB())));
        }
      }
    }
    // children:
