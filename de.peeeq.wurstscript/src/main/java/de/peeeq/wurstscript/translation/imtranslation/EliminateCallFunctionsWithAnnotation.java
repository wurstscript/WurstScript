...
public class EliminateCallFunctionsWithAnnotation {
  ...
  public static void process(ImProg prog) {
    ...
    Multimap<String, ImFunction> annotatedFunctions =
        collectAnnotatedFunctions(prog, calledAnnotations);
    rewriteSpecialCalls(specialCalls, annotatedFunctions);
  }

  private static void rewriteSpecialCalls(
      List<ImFunctionCall> specialCalls, Multimap<String, ImFunction> annotatedFunctions) {
    for (ImFunctionCall fc : specialCalls) {
      rewriteSpecialCall(fc, annotatedFunctions);
    }
  }

  private static void rewriteSpecialCall(
      ImFunctionCall fc, Multimap<String, ImFunction> annotatedFunctions) {
