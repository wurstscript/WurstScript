...
public class ClassTranslator {
  ...
  private static void translateInnerClasses(
      ClassOrModuleOrModuleInstanciation mi, ImTranslator translator) {
  ...
  private void addOnDestroyActions(
      ImFunction f, List<ImStmt> addTo, ClassOrModuleInstanciation c, ImVar thisVar) {
    ...
    if (c instanceof ClassDef) {
      ...
      if (extended != null) {
        ...
        addTo.add(
            ImFunctionCall(
                c, onDestroy, typeArgs, ImExprs(ImVarAccess(thisVar)), false, CallType.NORMAL));
      }
    }
  }
  ...
  private void createConstructFunc(ConstructorDef constr) {
    ...
    if (superConstr != null) {
      ...
      f.getBody()
          .add(ImFunctionCall(trace, superConstrFunc, typeArgs, arguments, false, CallType.NORMAL));
    }
    // call classInitFunc:
    ...
    f.getBody()
        .add(
            ImFunctionCall(
                ...
                classInitFunc,
                typeArguments,
                JassIm.ImExprs(JassIm.ImVarAccess(thisVar)),
                false,
                CallType.NORMAL));
  ...
  private void translateClassInitFunc() {
    ...
    classInitFunc =
        JassIm.ImFunction(
            classDef,
            translator.getNameFor(classDef) + "_init",
            ImTypeVars(),
            ImVars(thisVar),
            ImVoid(),
            ImVars(),
    ...
    for (Pair<ImVar, VarInitialization> i : translator.getDynamicInits(classDef)) {
      ...
      if (i.getB() instanceof Expr) {
        ...
        ImStmt s =
            ImSet(
                trace,
                ImMemberAccess(trace, ImVarAccess(thisVar), ImTypeArguments(), v, ImExprs()),
                e.imTranslateExpr(translator, f));
