...
public class StmtTranslation {
  ...
  public static void translate(ImExitwhen s, List<LuaStatement> res, LuaTranslator tr) {
    LuaIf r =
        LuaAst.LuaIf(
            s.getCondition().translateToLua(tr),
    ...
    if (s.getRight().attrTyp() instanceof ImTupleType) {
      ...
      right =
          LuaAst.LuaExprFunctionCall(
              ExprTranslation.getTupleCopyFunc(tt, tr), LuaAst.LuaExprlist(right));
    }
  ...
  public static void translate(ImVarargLoop loop, List<LuaStatement> res, LuaTranslator tr) {
    ...
    res.add(
        LuaAst.LuaAssignment(
            LuaAst.LuaExprVarAccess(loopVar),
            LuaAst.LuaExprArrayAccess(
                LuaAst.LuaLiteral("__args"), LuaAst.LuaExprlist(LuaAst.LuaExprVarAccess(i)))));
