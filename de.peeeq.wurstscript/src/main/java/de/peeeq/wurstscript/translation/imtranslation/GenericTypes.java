...
class GenericTypes {
  ...
  public GenericTypes(List<ImTypeArgument> typeArguments) {
    for (ImTypeArgument ta : typeArguments) {
      Preconditions.checkArgument(
          !EliminateGenerics.isGenericType(ta.getType()),
          "Type arguments must not be generic: " + typeArguments);
    }
  ...
  public GenericTypes take(int n) {
    return new GenericTypes(typeArguments.stream().limit(n).collect(Collectors.toList()));
  }

  public GenericTypes drop(int n) {
    return new GenericTypes(typeArguments.stream().skip(n).collect(Collectors.toList()));
  }
