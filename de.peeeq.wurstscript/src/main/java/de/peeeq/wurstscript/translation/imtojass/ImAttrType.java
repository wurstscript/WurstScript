...
public class ImAttrType {
  ...
  public static ImType substituteType(
      ImType type, List<ImTypeArgument> generics, List<ImTypeVar> typeVars) {
    return type.match(
  ...
  public static ImType getType(ImMethodCall mc) {
    ...
    returnType =
        substituteType(
            returnType,
            mc.getTypeArguments(),
            mc.getMethod().getImplementation().getTypeVariables());
    ImType rt = mc.getReceiver().attrTyp();
    if (rt instanceof ImClassType) {
      ImClassType ct = (ImClassType) rt;
      returnType =
          substituteType(returnType, ct.getTypeArguments(), ct.getClassDef().getTypeVariables());
    }
    return returnType;
  }
  ...
  public static ImType getType(ImMemberAccess e) {
    ...
    if (receiverType1 instanceof ImClassType) {
      ...
      try {
        ...
        throw new RuntimeException(
            "Could not determine type of " + e + " with receiverType " + receiverType, ex);
      }
    } else {
      return t;
    }
