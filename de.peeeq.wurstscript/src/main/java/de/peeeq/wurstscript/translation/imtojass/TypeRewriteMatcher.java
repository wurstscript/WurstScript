...
public class TypeRewriteMatcher implements ImType.Matcher<ImType> {
  ...
  public ImType case_ImArrayTypeMulti(ImArrayTypeMulti t) {
    return JassIm.ImArrayTypeMulti(
        t.getEntryType().match(this), ImmutableList.copyOf(t.getArraySize()));
  }
  ...
  public ImType case_ImTupleType(ImTupleType t) {
    return JassIm.ImTupleType(
        t.getTypes().stream().map(tt -> tt.match(this)).collect(Collectors.toList()),
