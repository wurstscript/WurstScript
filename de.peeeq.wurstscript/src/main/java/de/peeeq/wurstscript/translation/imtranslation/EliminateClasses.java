...
public class EliminateClasses {
  ...
  private Set<String> specialNatives =
      ImmutableSet.of(TYPE_ID_TO_TYPE_NAME, MAX_TYPE_ID, INSTANCE_COUNT, MAX_INSTANCE_COUNT);
  ...
  private void createReflectionFunctions() {
    typeIdToTypeNameFunc =
        accessClassManagementVar(
            "typeIdToTypeName",
  ...
  private ImFunction maxTypeIdFunc() {
    ...
    ImFunction f =
        JassIm.ImFunction(
            ...
            imInt(),
            JassIm.ImVars(),
            JassIm.ImStmts(JassIm.ImReturn(prog.getTrace(), JassIm.ImIntVal(maxTypeId))),
  ...
  @NotNull
  private ImFunction accessClassManagementVar(
      String funcName,
      ImType returnType,
      ImExpr defaultReturn,
      Function<ImClass, ImExpr> makeAccess) {
  ...
  private ImStmts generateBinarySearch(
      int lower,
      int upper,
      ImVar typeId,
      ImClass[] typeIdToClass,
      Function<ImClass, ImExpr> makeAccess) {
    if (lower > upper) {
      return JassIm.ImStmts();
    } else if (lower == upper) {
      return JassIm.ImStmts(
          JassIm.ImReturn(prog.getTrace(), makeAccess.apply(typeIdToClass[lower])));
    } else {
  ...
  public void createDispatchFunc(ImClass c, ImMethod m) {
    ...
    List<Pair<IntRange, ImMethod>> ranges = calculateTypeIdRanges(c, methods);
    ...
    ImFunction df =
        JassIm.ImFunction(
            m.getTrace(),
            "dispatch_" + c.getName() + "_" + m.getName(),
            JassIm.ImTypeVars(),
            m.getImplementation().getParameters().copy(),
            m.getImplementation().getReturnType(),
            JassIm.ImVars(),
            JassIm.ImStmts(),
            flags);
    ...
    ImExpr typeId =
        JassIm.ImVarArrayAccess(
            m.getTrace(), mVars.typeId, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(thisVar)));
          ...
          .add(
              ...
              JassIm.ImIf(
                  df.getTrace(),
                      JassIm.ImOperatorCall(
                          WurstOperator.EQ,
                          JassIm.ImExprs((ImExpr) typeId.copy(), JassIm.ImIntVal(0))),
                  // then
                  // if this == 0
                  JassIm.ImStmts(
                          JassIm.ImIf(
                              ...
                              JassIm.ImOperatorCall(
                                  WurstOperator.EQ,
                                  JassIm.ImExprs(JassIm.ImVarAccess(thisVar), JassIm.ImIntVal(0))),
                              // then error(NPE)
                              JassIm.ImStmts(
                                  translator.imError(
                                      trace,
                                      JassIm.ImStringVal(
                                          "Nullpointer exception when calling "
                                              + c.getName()
                                              + "."
                                              + methodName))),
  ...
  private void createDispatch(
      ...
      ImVar resultVar,
      ImExpr typeId,
      List<Pair<IntRange, ImMethod>> ranges,
      int start,
      int end) {
    if (start == end) {
      ...
      ImFunctionCall call =
          JassIm.ImFunctionCall(
              df.getTrace(),
              ranges.get(start).getB().getImplementation(),
      ...
      ImExpr condition =
          JassIm.ImOperatorCall(
              WurstOperator.LESS_EQ,
              JassIm.ImExprs((ImExpr) typeId.copy(), JassIm.ImIntVal(ranges.get(mid).getA().end)));
      ...
      createDispatch(df, elseBlock, resultVar, typeId, ranges, mid + 1, end);
    }
  }
  ...
  /**
   * returns a mapping from classdefs to functions into a mapping from typeid ranges to functions
   *
   * <p>the resulting list is sorted by the intrange and the intervals are disjunct
   */
  ...
  private void calculateTypeIdToMethodHelper(
      ImClass c, List<ImMethod> methods, ImMethod current, Map<Integer, ImMethod> typeIdToMethod) {
  ...
  private void replaceInstanceof(ImInstanceof e) {
    ...
    List<Integer> subClassIds =
        allSubClasses.stream().map(ImClass::attrTypeId).collect(Collectors.toList());
    ...
    if (useTempVar) {
      newExpr =
          JassIm.ImStatementExpr(
              JassIm.ImStmts(JassIm.ImSet(f.getTrace(), JassIm.ImVarAccess(tempVar), objTypeId)),
              newExpr);
    }
    e.replaceBy(newExpr);
  }
  ...
  private ImExpr inRange(ImExpr obj, IntRange range) {
    if (range.start == range.end) {
      return JassIm.ImOperatorCall(
          WurstOperator.EQ, JassIm.ImExprs((ImExpr) obj.copy(), JassIm.ImIntVal(range.start)));
    } else {
      return JassIm.ImOperatorCall(
          WurstOperator.AND,
          JassIm.ImExprs(
              JassIm.ImOperatorCall(
                  WurstOperator.GREATER_EQ,
                  JassIm.ImExprs((ImExpr) obj.copy(), JassIm.ImIntVal(range.start))),
              JassIm.ImOperatorCall(
                  WurstOperator.LESS_EQ,
                  JassIm.ImExprs((ImExpr) obj.copy(), JassIm.ImIntVal(range.end)))));
    }
  }
  ...
  private void replaceMethodCall(ImMethodCall mc) {
    ...
    if (dispatch == null) {
      throw new CompileError(
          mc.attrTrace().attrSource(), "Could not find dispatch for " + mc.getMethod().getName());
    }
    mc.replaceBy(
        JassIm.ImFunctionCall(
            mc.getTrace(), dispatch, JassIm.ImTypeArguments(), arguments, false, CallType.NORMAL));
  }
