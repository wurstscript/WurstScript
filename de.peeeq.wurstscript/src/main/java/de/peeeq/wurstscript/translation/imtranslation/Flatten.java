...
/**
 * flattening expressions and statements after flattening there will be no more StatementExprs for
 * expressions there might be a StatementExpr on the top level
 *
 * <p>TODO wait, its not that easy: you have to make sure that the execution order is not changed
 * for functions and global variables
 *
 * <p>e.g. take
 *
 * <p>y = x + StatementExpr(setX(4), 2)
 *
 * <p>this should be translated to:
 *
 * <p>temp = x setX(4) y = temp + 2
 *
 * <p>
 *
 * <p>alternative: relax language semantics
 */
public class Flatten {

  public static Result flatten(
      ImTypeVarDispatch imTypeVarDispatch, ImTranslator translator, ImFunction f) {
  ...
  public static class Result {
    ...
    public Result(List<ImStmt> stmts, ImExpr expr) {
      ...
      Preconditions.checkArgument(
          stmts.stream().allMatch(s -> s.getParent() == null), "statement must not have a parent");
      this.stmts = stmts;
      this.expr = expr;
    }
  ...
  private static void exprToStatements(
      List<ImStmt> result, Element e, ImTranslator t, ImFunction f) {
  ...
  private static void flattenStatementsInto(
      List<ImStmt> result, ImStmts statements, ImTranslator t, ImFunction f) {
  ...
  public static Result flatten(ImFunctionCall e, ImTranslator t, ImFunction f) {
    ...
    return new Result(
        ...
        ImFunctionCall(
            e.getTrace(),
            e.getFunc(),
            e.getTypeArguments().copy(),
            ImExprs(r.exprs),
            e.getTuplesEliminated(),
            e.getCallType()));
  }
      ...
      case AND:
        {
          ...
          if (right.stmts.isEmpty()) {
            return new Result(
                left.stmts,
                JassIm.ImOperatorCall(WurstOperator.AND, ImExprs(left.expr, right.expr)));
          } else {
            ArrayList<ImStmt> stmts = Lists.newArrayList(left.stmts);
            ImVar tempVar =
                JassIm.ImVar(
                    e.attrTrace(), WurstTypeBool.instance().imTranslateType(t), "andLeft", false);
            ...
            ImStmts elseBlock =
                JassIm.ImStmts(ImSet(trace, ImVarAccess(tempVar), JassIm.ImBoolVal(false)));
      ...
      case OR:
        {
          ...
          if (right.stmts.isEmpty()) {
            return new Result(
                left.stmts,
                JassIm.ImOperatorCall(WurstOperator.OR, ImExprs(left.expr, right.expr)));
          } else {
            ArrayList<ImStmt> stmts = Lists.newArrayList(left.stmts);
            ImVar tempVar =
                JassIm.ImVar(trace, WurstTypeBool.instance().imTranslateType(t), "andLeft", false);
            f.getLocals().add(tempVar);
            // if left is true then result is ture
            ImStmts thenBlock =
                JassIm.ImStmts(ImSet(trace, ImVarAccess(tempVar), JassIm.ImBoolVal(true)));
            // else check right
  ...
  public static ResultL flattenL(ImVarArrayAccess e, ImTranslator t, ImFunction f) {
    ...
    return new ResultL(
        indexes.stmts, ImVarArrayAccess(e.getTrace(), e.getVar(), ImExprs(indexes.exprs)));
  }
  ...
  public static Result flatten(ImMemberAccess e, ImTranslator t, ImFunction f) {
    ...
    Result res =
        new Result(
            rr.stmts,
            ImMemberAccess(
                e.getTrace(), rr.expr, e.getTypeArguments().copy(), e.getVar(), ImExprs(ir.exprs)));
  ...
  public static ResultL flattenL(ImMemberAccess e, ImTranslator t, ImFunction f) {
    ...
    ResultL res =
        new ResultL(
            rr.stmts,
            ImMemberAccess(
                e.getTrace(), rr.expr, e.getTypeArguments(), e.getVar(), ImExprs(ir.exprs)));
    res.addStmts(ir.stmts);
    return res;
  }
  ...
  public static Result flatten(ImVarargLoop s, ImTranslator translator, ImFunction f) {
    return new Result(
        Collections.singletonList(
            JassIm.ImVarargLoop(
                s.getTrace(), flattenStatements(s.getBody(), translator, f), s.getLoopVar())));
  }
}
