...
public class ImTranslator {
  ...
  private ImVar lastInitFunc =
      JassIm.ImVar(
          emptyTrace, WurstTypeString.instance().imTranslateType(this), "lastInitFunc", false);

  private int compiletimeOrderCounter = 1;
  private final Map<TranslatedToImFunction, FunctionFlagCompiletime> compiletimeFlags =
      new HashMap<>();
    ...
    try {
      ...
      debugPrintFunction =
          ImFunction(
              ...
              ImVars(
                  JassIm.ImVar(
                      wurstProg, WurstTypeString.instance().imTranslateType(this), "msg", false)),
              ImVoid(),
              ImVars(),
              ImStmts(),
              flags(IS_NATIVE, IS_BJ));
      ...
      if (mainFunc == null) {
        mainFunc =
            ImFunction(
                emptyTrace, "main", ImTypeVars(), ImVars(), ImVoid(), ImVars(), ImStmts(), flags());
      ...
      throw new RuntimeException(
          "There was a Wurst bug in the translation of "
  ...
  /**
   * Number all the compiletime functions and expressions, so that the one with the lowest number
   * can be executed first.
   *
   * <p>Dependendend packages are executed first and inside a package it goes from top to bottom.
   */
  ...
  private void calculateCompiletimeOrder_walk(WPackage p, Set<WPackage> visited) {
    ...
    p.accept(
        new de.peeeq.wurstscript.ast.Element.DefaultVisitor() {
          @Override
      ...
      if (f.isNative() && natives.containsKey(f.getName())) {
        ...
        if (!compatibleTypes(f, existing)) {
          throw new CompileError(
              f,
              "Native function definition conflicts with other native function defined in "
  ...
  private void finishInitFunctions() {
    ...
    getMainFunc()
        ...
        .add(
            0,
            ImFunctionCall(
                emptyTrace, globalInitFunc, ImTypeArguments(), ImExprs(), false, CallType.NORMAL));
      ...
      getMainFunc()
          ...
          .add(
              JassIm.ImFunctionCall(
                  emptyTrace,
                  native_DestroyTrigger,
                  ImTypeArguments(),
                  JassIm.ImExprs(JassIm.ImVarAccess(initTrigVar)),
                  false,
                  CallType.NORMAL));
    }
  }
  ...
  private ImVar prepareTrigger() {
    ...
    if (createTrigger != null) {
      getMainFunc()
          ...
          .add(
              ImSet(
                  getMainFunc().getTrace(),
                  ImVarAccess(initTrigVar),
                  JassIm.ImFunctionCall(
                      getMainFunc().getTrace(),
        ...
        ImFunctionCall(
            ...
            JassIm.ImExprs(
                ...
                ImFunctionCall(
                    ...
                    native_Condition,
                    ImTypeArguments(),
                    JassIm.ImExprs(JassIm.ImFuncRef(trace, initFunc)),
                    false,
                    CallType.NORMAL)),
            true,
            CallType.NORMAL));
        ...
        JassIm.ImIf(
            ...
            JassIm.ImOperatorCall(
                ...
                JassIm.ImExprs(
                    ImFunctionCall(
                        trace,
                        native_TriggerEvaluate,
                        ImTypeArguments(),
                        JassIm.ImExprs(JassIm.ImVarAccess(initTrigVar)),
                        false,
                        CallType.NORMAL))),
            // then: DisplayTimedTextToPlayer(GetLocalPlayer(), 0., 0., 45., "Could not initialize
            // package")
            JassIm.ImStmts(
                imError(
                    trace,
  ...
  public void addGlobalInitalizer(
      ImVar v, PackageOrGlobal packageOrGlobal, VarInitialization initialExpr) {
      ...
      for (int i = 0; i < arInit.getValues().size(); i++) {
        ...
        f.getBody()
            .add(
                ImSet(
                    trace,
                    ImVarArrayAccess(trace, v, ImExprs((ImExpr) JassIm.ImIntVal(i))),
                    translated));
      }
  ...
  public GetAForB<StructureDef, ImFunction> destroyFunc =
      new GetAForB<StructureDef, ImFunction>() {
        ...
        public ImFunction initFor(StructureDef classDef) {
          ...
          ImFunction f =
              ImFunction(
                  classDef.getOnDestroy(),
                  "destroy" + classDef.getName(),
                  ImTypeVars(),
                  params,
                  TypesHelper.imVoid(),
                  ImVars(),
  ...
  public GetAForB<StructureDef, ImMethod> destroyMethod =
      new GetAForB<StructureDef, ImMethod>() {
  ...
  private ImType selfType(TranslatedToImFunction f) {
    return f.match(
        new TranslatedToImFunction.Matcher<ImType>() {
          @Override
  ...
  public GetAForB<ImClass, ImFunction> allocFunc =
      new GetAForB<ImClass, ImFunction>() {
  ...
  public GetAForB<ImClass, ImFunction> deallocFunc =
      new GetAForB<ImClass, ImFunction>() {
        ...
        public ImFunction initFor(ImClass c) {
          ...
          return ImFunction(
              ...
              "dealloc_" + c.getName(),
              ImTypeVars(),
              JassIm.ImVars(JassIm.ImVar(c.getTrace(), TypesHelper.imInt(), "obj", false)),
              TypesHelper.imVoid(),
              JassIm.ImVars(),
              JassIm.ImStmts(),
              Collections.<FunctionFlag>emptyList());
  ...
  private final GetAForB<TypeParamDef, ImTypeVar> typeVariable =
      new GetAForB<TypeParamDef, ImTypeVar>() {
  ...
  public ImFunction getFuncFor(TranslatedToImFunction funcDef) {
    ...
    ImFunction f =
        ImFunction(funcDef, name, typeVars, ImVars(), ImVoid(), ImVars(), ImStmts(), flags);
  ...
  private ImClass getClassForFunc(TranslatedToImFunction funcDef) {
    ...
    return funcDef.match(
        new TranslatedToImFunction.Matcher<ImClass>() {
          @Override
  ...
  private ImTypeVars collectTypeVarsForFunction(TranslatedToImFunction funcDef) {
    ...
    funcDef.match(
        new TranslatedToImFunction.MatcherVoid() {
          @Override
  ...
  public ImFunction getInitFuncFor(WPackage p) {
    ...
    return initFuncMap.computeIfAbsent(
        p,
        p1 ->
            ImFunction(
                p1,
  ...
  /** returns a suitable name for the given element the returned name is a valid jass identifier */
  ...
  public Map<ClassDef, FuncDef> getClassesWithImplementation(
      Collection<ClassDef> instances, FuncDef func) {
  ...
  private Map<ClassDef, List<Pair<ImVar, VarInitialization>>> classDynamicInitMap =
    ...
    if (f == null) {
      ...
      f =
          ImFunction(
              ...
              ImTypeVars(),
              ImVars(),
              selfType(constr.attrNearestClassOrInterface()),
              ImVars(),
              ImStmts(),
              flags());
      addFunction(f, constr);
  ...
  /** calculates list of all classes ignoring the ones in modules, only module instantiations */
   ...
   * @param typeConstructor how the types are constructed (creating an array or multi-array, just
   *     returning the type)
   * @param tr trace for the variables
   * @return
   */
  private VarsForTupleResult createVarsForType(
      String name,
      final ImType type,
      Function<ImType, ImType> typeConstructor,
      de.peeeq.wurstscript.ast.Element tr) {
    return type.match(
        new ImType.Matcher<VarsForTupleResult>() {
          @Override
          public VarsForTupleResult case_ImArrayType(ImArrayType at) {
            if (at.getEntryType() instanceof ImTupleType) {
              ...
              for (ImType t : tt.getTypes()) {
                ts.add(
                    createVarsForType(
                        name + "_" + tt.getNames().get(i), t, JassIm::ImArrayType, tr));
                i++;
              }
          ...
          public VarsForTupleResult case_ImArrayTypeMulti(ImArrayTypeMulti at) {
            if (at.getEntryType() instanceof ImTupleType) {
              ...
              for (ImType t : tt.getTypes()) {
                ts.add(
                    createVarsForType(
                        name + "_" + tt.getNames().get(i),
                        t,
                        et -> JassIm.ImArrayTypeMulti(et, new ArrayList<>(at.getArraySize())),
                        tr));
                i++;
              }
              return new TupleResult(ts.build());
  ...
  private void addVarsForType(
      List<ImVar> result, String name, ImType type, de.peeeq.wurstscript.ast.Element tr) {
  ...
  public ImClass getClassForClosure(ExprClosure s) {
    ...
    return classForClosure.computeIfAbsent(
        s,
        s1 ->
            JassIm.ImClass(
                s1,
  ...
  public ImClass getClassFor(ClassOrInterface s) {
    Preconditions.checkNotNull(s);
    return classForStructureDef.computeIfAbsent(
        s,
        s1 -> {
  ...
  public Map<ImClass, ClassManagementVars> getClassManagementVars() {
    ...
    for (ImClass c : imProg.getClasses()) {
      ...
      ClassManagementVars v =
          classManagementVars.computeIfAbsent(rep, r -> new ClassManagementVars(r, this));
  ...
  private ImFunction makeDefaultErrorFunc() {
    ...
    ImExpr msg =
        JassIm.ImOperatorCall(
            WurstOperator.PLUS,
            ImExprs(
                JassIm.ImVarAccess(msgVar),
                JassIm.ImOperatorCall(
                    WurstOperator.PLUS,
                    ImExprs(JassIm.ImStringVal("\n"), JassIm.ImGetStackTrace()))));

    body.add(
        ImFunctionCall(
            emptyTrace,
            getDebugPrintFunction(),
            ImTypeArguments(),
            JassIm.ImExprs(msg),
            false,
            CallType.NORMAL));
    ...
    //		stmts.add(JassAst.JassStmtCall("I2S",
    // JassAst.JassExprlist(JassAst.JassExprBinary(JassAst.JassExprIntVal("1"), JassAst.JassOpDiv(),
    // Jas
    //
    ...
    ImFunction errorFunc =
        ImFunction(emptyTrace, "error", ImTypeVars(), parameters, returnType, locals, body, flags);
