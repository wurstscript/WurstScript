...
public class StmtTranslation {
  ...
  public static ImStmt translate(LocalVarDef s, ImTranslator t, ImFunction f) {
    ...
    if (s.getInitialExpr() instanceof Expr) {
      ...
      for (int i = 0; i < ai.getValues().size(); i++) {
        ...
        stmts.add(
            ImSet(s, ImVarArrayAccess(s, v, ImExprs((ImExpr) JassIm.ImIntVal(i))), translatedExpr));
      }
              ...
              JassIm.ImOperatorCall(
                  ...
                  JassIm.ImExprs(
                      ImFunctionCall(
                          ...
                          CallType.NORMAL)))));
      // elem = next()
      ImFunctionCall nextCall =
          ImFunctionCall(
              s, nextFuncIm, ImTypeArguments(), fromTarget.copy(), false, CallType.NORMAL);

      WurstType nextReturn = nextFunc.getReturnType();
      ImExpr nextCallWrapped =
          ExprTranslation.wrapTranslation(s, t, nextCall, nextReturn, loopVarType);
  ...
  public static ImStmt translate(StmtForIn forIn, ImTranslator t, ImFunction f) {
    ...
    if (iteratorFuncOpt.isPresent() && nextFuncOpt.isPresent() && hasNextFuncOpt.isPresent()) {
      ...
      ImVar iteratorVar =
          JassIm.ImVar(forIn.getLoopVar(), iteratorCall.attrTyp(), "iterator", false);
      ...
      WurstType nextReturn = nextFunc.getReturnType();
      ImExpr nextCallWrapped =
          ExprTranslation.wrapTranslation(forIn, t, nextCall, nextReturn, loopVarType);
  ...
  /**
   * Translate a for in vararg loop. Unlike the other for loops we don't need an iterator etc.
   * because the loop is unrolled in the VarargEliminator
   */
  ...
  private static ImStmt case_StmtForRange(
      ImTranslator t,
      ImFunction f,
      LocalVarDef loopVar,
      Expr to,
      Expr step,
      WStatements body,
      WurstOperator opStep,
      WurstOperator opCompare,
      Element trace) {
    ...
    imBody.add(
        ImExitwhen(trace, ImOperatorCall(opCompare, ImExprs(ImVarAccess(imLoopVar), toExpr))));
    // loop body:
    ...
    imBody.add(
        ImSet(
            trace,
            ImVarAccess(imLoopVar),
            ImOperatorCall(opStep, ImExprs(ImVarAccess(imLoopVar), stepExpr))));
    result.add(ImLoop(trace, imBody));
    return ImHelper.statementExprVoid(ImStmts(result));
  }

  private static ImExpr addCacheVariableSmart(
      ImTranslator t, ImFunction f, List<ImStmt> result, Expr toCache, ImType type) {
  ...
  public static ImStmt translate(StmtIf s, ImTranslator t, ImFunction f) {
    return ImIf(
        s,
        s.getCond().imTranslateExpr(t, f),
        ImStmts(t.translateStatements(f, s.getThenBlock())),
        ImStmts(t.translateStatements(f, s.getElseBlock())));
  }
  ...
  public static ImStmt translate(StmtWhile s, ImTranslator t, ImFunction f) {
    ...
    body.add(
        ImExitwhen(
            s.getCond(),
            ImOperatorCall(WurstOperator.NOT, ImExprs(s.getCond().imTranslateExpr(t, f)))));
  ...
  public static ImStmt translate(SwitchStmt switchStmt, ImTranslator t, ImFunction f) {
    ...
    for (int i = 0; i < switchStmt.getCases().size(); i++) {
      ...
      if (lastIf == null) {
        lastIf =
            ImIf(
                switchStmt,
                translateSwitchCase(cse, tempVar, f, t),
                ImStmts(t.translateStatements(f, cse.getStmts())),
                ImStmts());
        result.add(lastIf);
    ...
    if (switchStmt.getSwitchDefault() instanceof SwitchDefaultCaseStatements) {
      ...
      SwitchDefaultCaseStatements dflt =
          (SwitchDefaultCaseStatements) switchStmt.getSwitchDefault();
   ...
   * <p>is translated to
   *
   * <p>tempVar == x or tempVar == y or tempVar == z
   */
  private static ImExpr translateSwitchCase(
      SwitchCase cse, ImExpr tempVar, ImFunction f, ImTranslator t) {
    return cse.getExpressions().stream()
        .<ImExpr>map(
            e -> ImOperatorCall(WurstOperator.EQ, ImExprs(tempVar.copy(), e.imTranslateExpr(t, f))))
        .reduce((x, y) -> ImOperatorCall(WurstOperator.OR, ImExprs(x, y)))
        .orElseGet(() -> JassIm.ImBoolVal(true));
  }

  public static ImStmt translate(
      EndFunctionStatement endFunctionStatement, ImTranslator translator, ImFunction f) {
    return ImHelper.nullExpr();
  }

  public static ImStmt translate(
      StartFunctionStatement startFunctionStatement, ImTranslator translator, ImFunction f) {
    return ImHelper.nullExpr();
  }
