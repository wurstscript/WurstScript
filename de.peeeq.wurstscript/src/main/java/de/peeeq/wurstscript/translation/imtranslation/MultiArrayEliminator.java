...
public class MultiArrayEliminator {
        ...
        if (arraySize.size() == 2) {
          ...
          for (int i = 0; i < size0; i++) {
            ImVar newVar =
                JassIm.ImVar(
                    v.getTrace(),
                    JassIm.ImArrayType(type.getEntryType()),
                    v.getName() + "_" + i,
                    false);
          ...
          if (getSetMap.containsKey(va.getVar())) {
            ...
            set.replaceBy(
                JassIm.ImFunctionCall(
                    set.getTrace(),
                    getSetMap.get(va.getVar()).setter,
          ...
          args.add(
              JassIm.ImStringVal(
                  "when reading array "
                      + am.getVar().getName()
                      + " in "
                      + StackTraceInjector2.getCallPos(am.getTrace().attrSource())));
        }
        if (getSetMap.containsKey(am.getVar())) {
          am.replaceBy(
              JassIm.ImFunctionCall(
                  am.attrTrace(),
                  getSetMap.get(am.getVar()).getter,
                  JassIm.ImTypeArguments(),
                  args,
                  false,
                  CallType.NORMAL));
  ...
  private ImFunction generateSetFunc(ImVar aVar, List<ImVar> newArrays) {
    ...
    ImExpr highCond =
        JassIm.ImOperatorCall(
            ...
            JassIm.ImExprs(
                JassIm.ImVarAccess(arrayIndex), JassIm.ImIntVal(mtype.getArraySize().get(0))));
    ImExpr lowCond =
        JassIm.ImOperatorCall(
            WurstOperator.LESS, JassIm.ImExprs(JassIm.ImVarAccess(arrayIndex), JassIm.ImIntVal(0)));
    ...
    ImFunction setFunc =
        JassIm.ImFunction(
            ...
            aVar.getName() + "_set",
            JassIm.ImTypeVars(),
            JassIm.ImVars(instanceId, arrayIndex, value),
            JassIm.ImVoid(),
            locals,
            body,
            Lists.<FunctionFlag>newArrayList());
    if (generateStacktraces) {
  ...
  private void generateBinSearchSet(
      ImStmts stmts,
      ImVar indexVar1,
      ImVar indexVar2,
      ImVar value,
      List<ImVar> newArrays,
      int start,
      int end,
      ...
      stmts.add(
          JassIm.ImSet(
              ...
              JassIm.ImVarArrayAccess(
                  trace,
                  newArrays.get(start),
                  JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(indexVar1))),
  ...
  private ImFunction generateGetFunc(ImVar aVar, List<ImVar> newArrays) {
    ...
    ImFunction getFunc =
        JassIm.ImFunction(
            ...
            aVar.getName() + "_get",
            JassIm.ImTypeVars(),
            JassIm.ImVars(instanceId, arrayIndex),
            mtype.getEntryType(),
            locals,
            body,
            Lists.<FunctionFlag>newArrayList());
  ...
  private void generateBinSearchGet(
      ImStmts stmts,
      ImVar indexVar1,
      ImVar indexVar2,
      ImVar resultVar,
    ...
    if (start == end) {
      ...
      generateBinSearchGet(
          thenBlock, indexVar1, indexVar2, resultVar, newArrays, start, mid, trace);
      generateBinSearchGet(
          elseBlock, indexVar1, indexVar2, resultVar, newArrays, mid + 1, end, trace);
    }
  }
}
