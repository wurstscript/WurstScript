...
public class StackTraceInjector2 {
        ...
        new Element.DefaultVisitor() {
          ...
          public void visit(ImMethodCall call) {
            ...
            if (affectedFuncs.contains(call.getMethod().getImplementation())) {
              ...
              call.getArguments()
                  .add(
                      getStacktraceIndex(call),
                      str("when calling " + name(call.getMethod()) + "" + callPos));
            }
          }
        });
  }
  ...
  private void addStackTracePush(
      Multimap<ImFunction, ImFunctionCall> calls, Set<ImFunction> affectedFuncs) {
      ...
      stmts.add(
          ...
          JassIm.ImSet(
              ...
              JassIm.ImVarArrayAccess(
                  trace, stack, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(stackSize))),
              getStackPosVar(f)));
    }
  }

  private ImExpr getStackPosVar(ImFunction f) {
    return JassIm.ImVarAccess(
            ...
            .orElseGet(
                () -> {
                  throw new CompileError(
                      f, "Function " + f.getName() + " has no stacktrace parameter.");
                }));
  }
  ...
  private void passStacktraceParams(
    ...
    for (ImFunction f : affectedFuncs) {
      ...
      f.getParameters()
          .add(
              getStacktraceIndex(f),
              JassIm.ImVar(f.getTrace(), TypesHelper.imString(), STACK_POS_PARAM, false));
      // pass the stacktrace parameter at all calls
      for (ImFunctionCall call : callsForF) {
        String callPos = getCallPos(call.attrTrace().attrErrorPos());
        call.getArguments()
            .add(getStacktraceIndex(call), str("when calling " + name(f) + "" + callPos));
      }
    }
  }
  ...
  private ImStmt increment(de.peeeq.wurstscript.ast.Element trace, ImVar v) {
    return JassIm.ImSet(
        trace,
        JassIm.ImVarAccess(v),
        JassIm.ImOperatorCall(
            WurstOperator.PLUS, JassIm.ImExprs(JassIm.ImVarAccess(v), JassIm.ImIntVal(1))));
  }
  ...
  private ImStmt decrement(de.peeeq.wurstscript.ast.Element trace, ImVar v) {
    return JassIm.ImSet(
        ...
        JassIm.ImVarAccess(v),
        JassIm.ImOperatorCall(
            WurstOperator.MINUS, JassIm.ImExprs(JassIm.ImVarAccess(v), JassIm.ImIntVal(1))));
  }

  private void rewriteFuncRefs(
      final List<ImFuncRefOrCall> funcRefs, Set<ImFunction> affectedFuncs) {
    // rewrite funcrefs
    for (ImFuncRefOrCall fr : funcRefs) {
      ...
      ImFunctionCall call =
          JassIm.ImFunctionCall(frTrace, f, JassIm.ImTypeArguments(), args, true, CallType.NORMAL);
      ...
      loopBody.add(
          JassIm.ImExitwhen(
              ...
              JassIm.ImOperatorCall(
                  ...
                  JassIm.ImExprs(
                      JassIm.ImVarAccess(traceLimit), JassIm.ImIntVal(MAX_STACKTRACE_SIZE)))));
      // exitwhen i < 0
      loopBody.add(
          JassIm.ImExitwhen(
              trace,
                      ...
                      JassIm.ImOperatorCall(
                          ...
                          JassIm.ImExprs(
                              ...
                              JassIm.ImVarArrayAccess(
                                  trace,
                                  stack,
                                  JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(traceI)))))))));
