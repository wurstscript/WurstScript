...
/**
 * Takes a program and eliminates vararg functions, replacing them with generated functions with the
...
public class VarargEliminator {
  ...
  /**
   * Generates a function based on the vararg function with the appropriate amount of parameters for
   * the function call.
   */
  private void generateVarargFunc(ImFunction func, int numberOfParams) {
    ...
    // Remove vararg flag
    newFunc.setFlags(
        newFunc.getFlags().stream().filter(flag -> flag != IS_VARARG).collect(Collectors.toList()));
  ...
  private void redirectCall(ImFunctionCall call, ImFunction newFunc) {
    ...
    ImFunctionCall newCall =
        JassIm.ImFunctionCall(
            ...
            JassIm.ImExprs(call.getArguments().removeAll()),
