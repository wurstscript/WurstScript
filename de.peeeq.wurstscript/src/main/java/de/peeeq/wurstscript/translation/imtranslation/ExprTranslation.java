...
public class ExprTranslation {
  ...
  static ImExpr wrapTranslation(
      Element trace,
      ImTranslator t,
      ImExpr translated,
      WurstType actualType,
      WurstType expectedTypRaw) {
    if (t.isLuaTarget()) {
    ...
    if (toIndex != null && fromIndex != null) {
      ...
      return ImFunctionCall(
          trace, fromIndex, ImTypeArguments(), JassIm.ImExprs(translated), false, CallType.NORMAL);
    } else if (toIndex != null) {
      //            System.out.println("  --> toIndex");
      return ImFunctionCall(
          trace, toIndex, ImTypeArguments(), JassIm.ImExprs(translated), false, CallType.NORMAL);
    }
    return translated;
  }
  ...
  public static ImExpr translateIntern(ExprBinary e, ImTranslator t, ImFunction f) {
    ...
    if (e.attrFuncLink() != null) {
      ...
      return ImFunctionCall(
          e, calledFunc, ImTypeArguments(), ImExprs(left, right), false, CallType.NORMAL);
    }
  ...
  private static ImExpr translateNameDef(NameRef e, ImTranslator t, ImFunction f)
      throws CompileError {
    ...
    if (decl instanceof VarDef) {
      ...
      if (e.attrImplicitParameter() instanceof Expr) {
        ...
        if (e instanceof AstElementWithIndexes) {
          ...
          return JassIm.ImMemberAccess(
              e, index1, JassIm.ImTypeArguments(), v, JassIm.ImExprs(index2));
        } else {
      ...
      throw new CompileError(
          e.getSource(), "Cannot translate reference to " + Utils.printElement(decl));
    }
  }
  ...
  private static ImExpr translateTupleSelection(ImTranslator t, ImFunction f, ExprMemberVar mv) {
    ...
    if (left instanceof ImLExpr) {
      ...
      return JassIm.ImStatementExpr(
          JassIm.ImStmts(ImSet(left.attrTrace(), ImVarAccess(v), left)),
  ...
  */
  ...
  private static ImExpr translateFunctionCall(
      FunctionCall e, ImTranslator t, ImFunction f, boolean returnReveiver) {
    ...
    if (e.getFuncName().equals("ExecuteFunc")) {
      ...
      return ImFunctionCall(
          e, executedFunc, ImTypeArguments(), JassIm.ImExprs(), true, CallType.EXECUTE);
    }
    ...
    if (e.getFuncName().equals("compiletime")
      ...
      return JassIm.ImCompiletimeExpr(
          e, e.getArgs().get(0).imTranslateExpr(t, f), t.getCompiletimeExpressionsOrder(e));
    }
  ...
  private static ImTypeArguments getFunctionCallTypeArguments(
      ImTranslator tr, FunctionSignature sig, Element location, ImTypeVars typeVariables) {
    ...
    for (ImTypeVar tv : typeVariables) {
      ...
      if (to.isEmpty()) {
        throw new CompileError(
            location, "Type variable " + tp.getName() + " not bound in mapping.");
      }
  ...
  public static ImExpr translateIntern(ExprNewObject e, ImTranslator t, ImFunction f) {
    ...
    return ImFunctionCall(
        e, constructorImFunc, typeArgs, translateExprs(e.getArgs(), t, f), false, CallType.NORMAL);
  }
  ...
  public static ImExpr translateIntern(ExprInstanceOf e, ImTranslator translator, ImFunction f) {
    ...
    if (imTargetType instanceof ImClassType) {
      return JassIm.ImInstanceof(
          e.getExpr().imTranslateExpr(translator, f), (ImClassType) imTargetType);
    }
  ...
  public static ImExpr destroyClass(
      ExprDestroy s, ImTranslator t, ImFunction f, StructureDef classDef) {
    ImMethod destroyFunc = t.destroyMethod.getFor(classDef);
    return ImMethodCall(
        s,
        destroyFunc,
        ImTypeArguments(),
        s.getDestroyedObj().imTranslateExpr(t, f),
        ImExprs(),
        false);
  }
  ...
  public static ImLExpr translateLvalue(LExpr e, ImTranslator t, ImFunction f) {
    ...
    if (decl == null) {
      // should only happen with gg_ variables
      throw new CompileError(
          e.getSource(), "Translation Error: Could not find definition of " + e.getVarName() + ".");
    }
    if (decl instanceof VarDef) {
      VarDef varDef = (VarDef) decl;
  ...
  //    public static ImLExpr translateLvalue(ExprVarArrayAccess e, ImTranslator translator,
  // ImFunction f) {
  ...
  //    }
  //
  //    public static ImLExpr translateLvalue(ExprMemberVar e, ImTranslator translator, ImFunction
  // f) {
  ...
  //    }
  //
  //    public static ImLExpr translateLvalue(ExprMemberArrayVar e, ImTranslator translator,
  // ImFunction f) {
  //        throw new RuntimeException("TODO");
  //    }
