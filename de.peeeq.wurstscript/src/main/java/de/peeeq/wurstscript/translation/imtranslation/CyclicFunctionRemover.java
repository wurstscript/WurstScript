...
/** Removes cyclic functions from a program by putting cyclic functions into one big function */
public class CyclicFunctionRemover {
  ...
  private void removeCycle(List<ImFunction> funcs) {
    ...
    ImVar choiceVar =
        JassIm.ImVar(trace, WurstTypeInt.instance().imTranslateType(tr), "funcChoice", false);
  ...
  private void replaceCalls(
      List<ImFunction> funcs, ImFunction newFunc, Map<ImVar, ImVar> oldToNewVar, Element e) {
    // process children
    for (int i = 0; i < e.size(); i++) {
    ...
    if (e instanceof ImFuncRef) {
      ...
      if (funcs.contains(f)) {
        ...
        ImFunctionCall call =
            JassIm.ImFunctionCall(
                fr.attrTrace(), f, JassIm.ImTypeArguments(), arguments, true, CallType.NORMAL);
      ...
      if (funcs.contains(oldFunc)) {
        ...
        ImFunctionCall newCall =
            JassIm.ImFunctionCall(
                fc.getTrace(), newFunc, JassIm.ImTypeArguments(), arguments, true, CallType.NORMAL);
        ...
        if (oldFunc.getReturnType() instanceof ImVoid) {
          ...
          ret =
              JassIm.ImStatementExpr(
                  JassIm.ImStmts(newCall),
                  JassIm.ImVarAccess(getTempReturnVar(oldFunc.getReturnType())));
        }
        fc.replaceBy(ret);
      }
    }
    ...
    if (e instanceof ImReturn) {
      ...
      ImStmts stmts =
          JassIm.ImStmts(
              JassIm.ImSet(
                  r.getTrace(),
                  JassIm.ImVarAccess(getTempReturnVar(returnType)),
                  (ImExpr) returnValue),
  ...
  private void calculateNewParameters(
      List<ImFunction> funcs, List<ImVar> newParameters, Map<ImVar, ImVar> oldToNewVar) {
