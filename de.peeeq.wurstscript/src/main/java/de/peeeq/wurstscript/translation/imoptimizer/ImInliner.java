...
public class ImInliner {
  ...
  private ImFunction inlineFunctions(
      ImFunction f,
      Element parent,
      int parentI,
      Element e,
      boolean[] changed,
      Map<ImFunction, Integer> alreadyInlined) {
    // TODO maybe it would be smarter to first optimize the parameters and then try to optimize the
    // call itself ...
    if (e instanceof ImFunctionCall) {
      ...
      if (f != called && shouldInline(call, called)) {
        if (alreadyInlined.getOrDefault(called, 0) < 5) { // check maximum to ensure termination
          ...
          //					translator.removeCallRelation(f, called); // XXX is it safe to remove this call
          // relation?
          changed[0] = true;
    ...
    for (int i = 0; i < e.size(); i++) {
      ...
      while (true) {
        ...
        // otherwise check the same expression again, but remember what we already inlined and how
        // often:
  ...
  private boolean shouldInline(ImFunctionCall call, ImFunction f) {
    ...
    return inlinableFunctions.contains(f) && getRating(f) < threshold && !isRecursive(f);
  }
  ...
  private void collectInlinableFunctions() {
    for (ImFunction f : prog.getFunctions()) {
      if (f.hasFlag(FunctionFlagEnum.IS_COMPILETIME_NATIVE)
