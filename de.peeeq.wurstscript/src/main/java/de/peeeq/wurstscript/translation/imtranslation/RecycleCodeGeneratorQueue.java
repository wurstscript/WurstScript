...
/**
 * Manages object ids in a queue. This way the time each object is inactive is maximized and thus
 * errors should be easier to detect
 */
  ...
  public void createAllocFunc(ImTranslator translator, ImProg prog, ImClass c) {
    ...
    body.add(
        JassIm.ImIf(
            ...
            JassIm.ImOperatorCall(
                WurstOperator.EQ,
                JassIm.ImExprs(JassIm.ImVarAccess(mVars.freeCount), JassIm.ImIntVal(0))),
            thenBlock,
            elseBlock));
    ...
    thenBlock.add(
        JassIm.ImIf(
            ...
            JassIm.ImOperatorCall(
                WurstOperator.LESS,
                JassIm.ImExprs(JassIm.ImVarAccess(mVars.maxIndex), JassIm.ImIntVal(maxSize))),
            ifEnoughMemory,
            ifNotEnoughMemory));
    //         maxIndex = maxIndex + 1
    ifEnoughMemory.add(
        JassIm.ImSet(
            ...
            JassIm.ImOperatorCall(
                WurstOperator.PLUS,
                JassIm.ImExprs(JassIm.ImVarAccess(mVars.maxIndex), JassIm.ImIntVal(1)))));
    // 		   this = maxIndex
    ifEnoughMemory.add(
        JassIm.ImSet(tr, JassIm.ImVarAccess(thisVar), JassIm.ImVarAccess(mVars.maxIndex)));
    //	       typeId[this] = ...
    ifEnoughMemory.add(
        JassIm.ImSet(
            tr,
            JassIm.ImVarArrayAccess(
                tr, mVars.typeId, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(thisVar))),
            JassIm.ImIntVal(c.attrTypeId())));
    //     else:
    //         error("out of memory")
    ifNotEnoughMemory.add(
        translator.imError(
            c.getTrace(),
            JassIm.ImStringVal("Out of memory: Could not create " + c.getName() + ".")));
    //         this = 0
    ...
    elseBlock.add(
        JassIm.ImSet(
            tr,
            JassIm.ImVarAccess(mVars.freeCount),
            JassIm.ImOperatorCall(
                WurstOperator.MINUS,
                JassIm.ImExprs(JassIm.ImVarAccess(mVars.freeCount), JassIm.ImIntVal(1)))));
    //     this = free[freeCount]
    elseBlock.add(
        JassIm.ImSet(
            tr,
            ...
            JassIm.ImVarArrayAccess(
                tr, mVars.free, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(mVars.freeCount)))));
    //     typeId[this] = ...
    elseBlock.add(
        JassIm.ImSet(
            tr,
    ...
    body.add(
        JassIm.ImIf(
            ...
            JassIm.ImStmts(
                translator.imError(
                    c.getTrace(),
                    JassIm.ImStringVal("Double free: object of type " + c.getName()))),
            // else
            JassIm.ImStmts(
                // free[freeCount] = this
                JassIm.ImSet(
                    ...
                    JassIm.ImVarArrayAccess(
                        tr,
                        mVars.free,
                        JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(mVars.freeCount))),
                    JassIm.ImVarAccess(thisVar)),
                // freeCount++
                JassIm.ImSet(
                    tr,
