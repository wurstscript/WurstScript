...
public class RecycleCodeGeneratorQueue implements RecycleCodeGenerator {
    ...
    public void createAllocFunc(ImTranslator translator, ImProg prog, ImClass c) {
        ...
        //	       typeId[this] = ...
        ifEnoughMemory.add(JassIm.ImSet(tr, JassIm.ImVarArrayAccess(mVars.typeId, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(thisVar))), JassIm.ImIntVal(c.attrTypeId())));
        //     else:
        //         error("out of memory")
        ...
        elseBlock.add(JassIm.ImSet(tr, JassIm.ImVarAccess(mVars.freeCount), JassIm.ImOperatorCall(WurstOperator.MINUS, JassIm.ImExprs(JassIm.ImVarAccess(mVars.freeCount), JassIm
                .ImIntVal(1)))));
        //     this = free[freeCount]
        elseBlock.add(JassIm.ImSet(tr, JassIm.ImVarAccess(thisVar), JassIm.ImVarArrayAccess(mVars.free, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(mVars.freeCount)))));
        //     typeId[this] = ...
        elseBlock.add(JassIm.ImSet(tr, JassIm.ImVarArrayAccess(mVars.typeId, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(thisVar))), JassIm.ImIntVal(c.attrTypeId())));
        // endif
    ...
    public void createDeallocFunc(ImTranslator translator, ImProg prog, ImClass c) {
        ...
        body.add(JassIm.ImIf(tr,
                JassIm.ImOperatorCall(WurstOperator.EQ,
                        JassIm.ImExprs(JassIm.ImVarArrayAccess(mVars.typeId, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(thisVar))), JassIm.ImIntVal(0))),
                // then
                // error
                ...
                JassIm.ImStmts(
                        // free[freeCount] = this
                        JassIm.ImSet(tr, JassIm.ImVarArrayAccess(mVars.free, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(mVars.freeCount))), JassIm.ImVarAccess(thisVar)),
                        // freeCount++
                        JassIm.ImSet(tr, JassIm.ImVarAccess(mVars.freeCount), JassIm.ImOperatorCall(WurstOperator.PLUS, JassIm.ImExprs(JassIm.ImVarAccess(mVars.freeCount),
                                JassIm.ImIntVal(1)))),
                        // typeId[this] = 0
                        JassIm.ImSet(tr, JassIm.ImVarArrayAccess(mVars.typeId, JassIm.ImExprs((ImExpr) JassIm.ImVarAccess(thisVar))), JassIm.ImIntVal(0))
                )));
