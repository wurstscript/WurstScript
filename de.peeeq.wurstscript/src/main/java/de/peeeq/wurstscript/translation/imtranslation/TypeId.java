...
public class TypeId {
  ...
  private static final Comparator<ImClass> class_comparator =
      Comparator.comparing(ImClass::getName).thenComparing(TypeId::packageName);
  ...
  public static Map<ImClass, Integer> calculate(ImProg prog) {
    ...
    List<ImClass> classes =
        prog.getClasses().stream().sorted(class_comparator).collect(Collectors.toList());
  ...
  private static void assignIds(
      AtomicInteger count, Map<ImClass, Integer> result, List<ImClass> classes) {
  ...
  private static Multimap<ImClass, ImClass> calculateSubclasses(List<ImClass> classes) {
    ...
    for (ImClass c : classes) {
      c.getSuperClasses().stream()
          ...
          .forEach(superClass -> subClasses.put(superClass, c));
    }
    return subClasses;
  }
  ...
  private static void assignId(
      AtomicInteger count,
      Map<ImClass, Integer> result,
      ImClass c,
      Multimap<ImClass, ImClass> subClasses) {
  ...
  public static int get(ImClass c) {
    ...
    if (res == null) {
      throw new CompileError(
          c, "Could not get type-id for " + c.getName() + ImPrinter.smallHash(c));
    }
    return res;
  }
