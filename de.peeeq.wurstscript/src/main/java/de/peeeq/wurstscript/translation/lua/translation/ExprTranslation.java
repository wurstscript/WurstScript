...
public class ExprTranslation {
  ...
  public static LuaExpr translate(ImAlloc e, LuaTranslator tr) {
    ...
    return LuaAst.LuaExprMethodCall(LuaAst.LuaExprVarAccess(classVar), m, LuaAst.LuaExprlist());
  }
                    ...
                    LuaAst.LuaExprFunctionAbstraction(
                        ...
                        LuaAst.LuaStatements(
                            LuaAst.LuaAssignment(
                                LuaAst.LuaExprVarAccess(tempRes),
                                LuaAst.LuaExprFunctionCall(
                                    tr.luaFunc.getFor(e.getFunc()),
                                    LuaAst.LuaExprlist(LuaAst.LuaExprVarAccess(tempDots)))))),
                    //                        LuaAst.LuaLiteral("function(err) " + errorFuncName(tr)
                    // + "(tostring(err)) end")
                    LuaAst.LuaLiteral(
                        "function(err) xpcall(function() "
                            + callErrorFunc(tr, "tostring(err)")
                            + " end, function(err2) BJDebugMsg(\"error reporting error: \" .. tostring(err2)) BJDebugMsg(\"while reporting: \" .. tostring(err))  end) end")
  ...
  public static LuaExpr translate(ImFunctionCall e, LuaTranslator tr) {
    return LuaAst.LuaExprFunctionCall(
        tr.luaFunc.getFor(e.getFunc()), tr.translateExprList(e.getArguments()));
  }

  public static LuaExpr translate(ImInstanceof e, LuaTranslator tr) {
    return LuaAst.LuaExprFunctionCall(
        tr.instanceOfFunction,
        LuaAst.LuaExprlist(
  ...
  public static LuaExpr translate(ImMemberAccess e, LuaTranslator tr) {
    LuaExpr res =
        LuaAst.LuaExprFieldAccess(e.getReceiver().translateToLua(tr), e.getVar().getName());
  ...
  public static LuaExpr translate(ImMethodCall e, LuaTranslator tr) {
    return LuaAst.LuaExprMethodCall(
        e.getReceiver().translateToLua(tr),
        tr.luaMethod.getFor(e.getMethod()),
        tr.translateExprList(e.getArguments()));
  }
  ...
  public static LuaExpr translate(ImOperatorCall e, LuaTranslator tr) {
    if (e.getArguments().size() == 2) {
      ...
      if (e.getOp() == WurstOperator.PLUS
        ...
        return LuaAst.LuaExprFunctionCall(
            tr.stringConcatFunction, LuaAst.LuaExprlist(leftExpr, rightExpr));
      } else if (e.getOp() == WurstOperator.MOD_INT) {
        op = LuaAst.LuaOpMod();

        return LuaAst.LuaExprFunctionCallE(
            LuaAst.LuaLiteral("math.floor"),
  ...
  private static LuaFunction getTupleEqualsFunc(ImTupleType t, LuaTranslator tr) {
    Optional<TupleFunc> tfo =
        tr.tupleEqualsFuncs.stream().filter(f -> f.tupleType.equalsType(t)).findFirst();
    TupleFunc tf;
    if (tfo.isPresent()) {
      tf = tfo.get();
    } else {
      ...
      LuaFunction func =
          LuaAst.LuaFunction(tr.uniqueName("tupleEquals"), LuaAst.LuaParams(t1, t2), body);
  ...
  public static LuaFunction getTupleCopyFunc(ImTupleType t, LuaTranslator tr) {
    Optional<TupleFunc> tfo =
        tr.tupleCopyFuncs.stream().filter(f -> f.tupleType.equalsType(t)).findFirst();
    TupleFunc tf;
    if (tfo.isPresent()) {
      tf = tfo.get();
    } else {
      ...
      for (ImType type : t.getTypes()) {
        ...
        LuaExpr v =
            LuaAst.LuaExprArrayAccess(
                LuaAst.LuaExprVarAccess(t1), LuaAst.LuaExprlist(LuaAst.LuaExprIntVal("" + i)));
  ...
  private static LuaExpr translateEquals(
      LuaExpr leftExpr, LuaExpr rightExpr, ImType t, LuaTranslator tr) {
  ...
  public static LuaExpr translate(ImStatementExpr e, LuaTranslator tr) {
    ...
    return LuaAst.LuaExprFunctionCallE(
        LuaAst.LuaExprFunctionAbstraction(LuaAst.LuaParams(), body), LuaAst.LuaExprlist());
  }
  ...
  public static LuaExpr translate(ImTupleSelection e, LuaTranslator tr) {
    return LuaAst.LuaExprArrayAccess(
        e.getTupleExpr().translateToLua(tr),
        LuaAst.LuaExprlist(LuaAst.LuaExprIntVal("" + (1 + e.getTupleIndex()))));
  }
  ...
  public static LuaExpr translate(ImVarArrayAccess e, LuaTranslator tr) {
    ...
    return LuaAst.LuaExprArrayAccess(
        LuaAst.LuaExprVarAccess(tr.luaVar.getFor(e.getVar())), indexes);
  }
  ...
  public static LuaExpr translate(ImCast imCast, LuaTranslator tr) {
    ...
    if (TypesHelper.isIntType(imCast.getToType())
        && !TypesHelper.isIntType(imCast.getExpr().attrTyp())) {
