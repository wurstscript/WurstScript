...
public class EliminateGenerics {
  ...
  private Table<ImFunction, GenericTypes, ImFunction> specializedFunctions =
  ...
  private Multimap<ImClass, BiConsumer<GenericTypes, ImClass>> onSpecializedClassTriggers =
      HashMultimap.create();
  ...
  private static Iterable<ImClassType> superTypes(ImClassType ct) {
    ...
    return () ->
        ct.getClassDef().getSuperClasses().stream()
            .map(sc -> (ImClassType) transformType(sc, generics, typeVars))
            .iterator();
  }

  /** Removed methods and functions from classes and adds them to the main program. */
  ...
  private void moveFunctionsOutOfClass(ImClass c) {
    ...
    for (ImFunction f : functions) {
      ...
      List<ImTypeVar> newTypeVars =
          c.getTypeVariables().stream().map(ImTypeVar::copy).collect(Collectors.toList());
  ...
  private void adaptSubmethods(List<ImMethod> oldSubMethods, ImMethod newM) {
    ...
    for (ImMethod subMethod : oldSubMethods) {
      ...
      if (isGenericType(subClassT)) {
        onSpecializeClass(
            subClass,
            (subGenerics, specializedSubClass) -> {
  ...
  private void rewriteGenerics(Element element, GenericTypes generics, List<ImTypeVar> typeVars) {
    if (generics.getTypeArguments().size() != typeVars.size()) {
      throw new RuntimeException(
          "Rewrite generics with wrong sizes\n"
              + "generics: "
              + generics
              + "\n"
              + "typevars: "
  ...
  private static ImType transformType(
      ImType type, GenericTypes generics, List<ImTypeVar> typeVars) {
  ...
  private ImClass specializeClass(ImClass c, GenericTypes generics) {
    ...
    onSpecializedClassTriggers.get(c).forEach(consumer -> consumer.accept(generics, newC));
    return newC;
  }
    ...
    element.accept(
        new Element.DefaultVisitor() {
          ...
          public void visit(ImClass c) {
            ...
            genericsUses.add(
                () -> {
                  List<ImClassType> newSuperClasses =
                      c.getSuperClasses().stream()
                          .map(EliminateGenerics.this::specializeType)
                          .collect(Collectors.toList());
  ...
  static boolean containsTypeVariable(ImType type) {
    return type.match(
        new ImType.Matcher<Boolean>() {
          ...
          @Override
          public Boolean case_ImClassType(ImClassType t) {
            return t.getTypeArguments().stream().anyMatch(tt -> containsTypeVariable(tt.getType()));
          }
    ...
    return type.match(
        new TypeRewriteMatcher() {
          ...
          public ImType case_ImClassType(ImClassType t) {
            ...
            ImClass specializedClass =
                specializeClass(t.getClassDef(), new GenericTypes(newTypeArgs));
