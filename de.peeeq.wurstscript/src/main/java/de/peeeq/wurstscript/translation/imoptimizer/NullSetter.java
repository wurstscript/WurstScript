...
public class NullSetter {
  ...
  private Set<String> primitiveTypes =
      Sets.newHashSet("boolean", "integer", "string", "code", "real");
  ...
  private void optimizeFunc(final ImFunction f) {
    ...
    for (ImVar local : handleVars) {
      nullSetStmts.add(
          JassIm.ImSet(trace, JassIm.ImVarAccess(local), JassIm.ImNull(local.getType())));
    }
   ...
   *
   * <p>returns true if "parent" always returns
   *
   * @param f
   * @param handleVars
   * @param nullSetStmts
  ...
  private boolean optimizeChildren(
      final ImFunction f,
      final List<ImVar> handleVars,
      final List<ImStmt> nullSetStmts,
      final de.peeeq.wurstscript.ast.Element trace,
      Element parent) {
    for (int i = 0; i < parent.size(); i++) {
      ...
      if (elem instanceof ImReturn) {
        ...
        ImIf imIf = (ImIf) elem;
        boolean returnsThen =
            optimizeChildren(f, handleVars, nullSetStmts, trace, imIf.getThenBlock());
        boolean returnsElse =
            optimizeChildren(f, handleVars, nullSetStmts, trace, imIf.getElseBlock());
  ...
  private void handleReturnStmt(
    ...
    if (imReturn.getReturnValue() instanceof ImExpr) { // returns some value
      ...
      if (exprContainsVar(returnExpr, handleVars)) {
        ...
        ImVar tempReturn =
            JassIm.ImVar(
                imReturn.attrTrace(), returnExpr.attrTyp(), f.getName() + "tempReturn", false);
