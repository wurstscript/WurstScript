...
public class AntlrJassParseTreeTransformer {
  ...
  public AntlrJassParseTreeTransformer(
  ...
  public CompilationUnit transform(JassParser.CompilationUnitContext cu) {
    ...
    return Ast.CompilationUnit(
        new CompilationUnitInfo(this.cuErrorHandler), jassDecls, Ast.WPackages());
  }
  ...
  private NameRef transformExprVarAccess(JassParser.ExprVarAccessContext e) {
    if (e.indexes() == null || e.indexes().isEmpty()) {
      ...
      return Ast.ExprVarArrayAccess(
          source(e),
          Ast.Identifier(source(e.varname), e.varname.getText()),
  ...
  private ExprFunctionCall transformFunctionCall(JassParser.ExprFunctionCallContext c) {
    return Ast.ExprFunctionCall(
        source(c),
        Ast.Identifier(source(c.funcName), c.funcName.getText()),
        Ast.TypeExprList(),
        transformExprs(c.argumentList().exprList()));
  }
  ...
  private Expr transformExprPrimary(JassParser.JassExprPrimaryContext e) {
    if (e.atom != null) {
      ...
      if (e.indexes() != null && !e.indexes().isEmpty()) {
        ...
        return Ast.ExprVarArrayAccess(
            source(e), Ast.Identifier(source(e.varname), e.varname.getText()), index);
      } else {
        return Ast.ExprVarAccess(source(e), Ast.Identifier(source(e.varname), e.varname.getText()));
      }
  ...
  private FuncSig transformFuncSig(JassParser.JassFuncSignatureContext s) {
    ...
    return new FuncSig(
        Ast.Identifier(source(s.name), s.name.getText()), typeParams, formalParameters, returnType);
  }
  ...
  private WParameter transformFormalParameter(
      JassParser.FormalParamContext p, boolean makeConstant) {
    Modifiers modifiers = Ast.Modifiers();
    if (makeConstant) {
      modifiers.add(Ast.ModConstant(source(p).artificial()));
    }
    return Ast.WParameter(
        source(p),
        modifiers,
        transformTypeExpr(p.jassTypeExpr()),
        Ast.Identifier(source(p.name), p.name.getText()));
  }
