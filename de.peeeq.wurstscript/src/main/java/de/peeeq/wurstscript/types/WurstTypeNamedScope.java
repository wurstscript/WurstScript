...
public abstract class WurstTypeNamedScope extends WurstType {
  ...
  VariableBinding matchAgainstSupertypeIntern(
    ...
    if (obj instanceof WurstTypeNamedScope) {
      ...
      if (other.getDef() == this.getDef()) {
        return matchTypeParams(
            getTypeParameters(), other.getTypeParameters(), location, mapping, variablePosition);
      }
    }
    return null;
  }
  ...
  public abstract WurstType replaceTypeVars(List<WurstTypeBoundTypeParam> newTypes);
  ...
  public WurstType replaceTypeVarsUsingTypeArgs(TypeExprList typeArgs) {
    ...
    if (typeArgs.size() != typeParameters.size()) {
      typeArgs.addError(
          "Expected " + typeParameters.size() + " type arguments, but got " + typeArgs.size());
    }
  ...
  protected VariableBinding matchTypeParams(
      List<WurstTypeBoundTypeParam> list,
      List<WurstTypeBoundTypeParam> list2,
      @Nullable Element location,
      VariableBinding mapping,
      VariablePosition variablePosition) {
  ...
  /** get the name links available in this class or interface This includes inherited names */
