...
public abstract class WurstTypeClassOrInterface extends WurstTypeNamedScope {
  ...
  public WurstTypeClassOrInterface(
      List<WurstTypeBoundTypeParam> typeParameters, boolean isStaticRef) {
  ...
  /** Level in the type hierarchy. Root is at 0, subtypes one below max supertype */
  protected int level() {
  ...
  /** if this type has a single abstract method, return it. Otherwise return null; */
  public FuncLink findSingleAbstractMethod(Element context) {
    ...
    for (NameLink nl : nameLinks.values()) {
      if (nl instanceof FuncLink && nl.getDef().attrIsAbstract()) {
        ...
        for (DefLink other : nameLinks.get(nl.getName())) {
          if (other != nl
              ...
              && CheckHelper.isRefinement(
                  this.getTypeArgBinding(),
                  ((FuncLink) other).getDef(),
                  ((FuncLink) nl).getDef())) {
  ...
  VariableBinding matchAgainstSupertypeIntern(
    ...
    // direct match
    VariableBinding superMapping =
        super.matchAgainstSupertypeIntern(obj, location, mapping, variablePosition);
    ...
    for (WurstTypeClassOrInterface implementedInterface : directSupertypes()) {
      ...
      VariableBinding mapping2 =
          implementedInterface.matchAgainstSupertype(
              obj, location, mapping, VariablePosition.RIGHT);
      if (mapping2 != null) {
        return mapping2;
      }
    }
