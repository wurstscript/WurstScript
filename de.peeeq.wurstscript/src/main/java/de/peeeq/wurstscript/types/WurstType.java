...
public abstract class WurstType {
  ...
  //    public final VariableBinding matchAgainstSupertype(WurstType other, @Nullable Element
  // location) {
  //        return this.matchAgainstSupertype(other, location,
  // TreeMap.empty(TypeParamOrd.instance()));
  //    }

  /**
   * Matches this type against another type.
   *
   * <p>Will try to instantiate type variables from the set typeParams
   */
  public final @Nullable VariableBinding matchAgainstSupertype(
      WurstType other,
  ...
  /**
   * Matches a type against a supertype . Both sides can include type parameters, but only the type
   * parameters given in typeParams are matched.
   *
   * <p>returns the type mapping required to make the types equal and null if they do not match.
   *
   * <p>Simple examples: int match: int --> {} (the empty mapping} int match: string --> null ⋀T.
   * A[int] match: A[T] --> {T -> int} ⋀T. A[T] match A[int] --> {T -> int}
   *
   * <p>Nonlinear patterns are also possible (matches from left to right): ⋀T. A[T, T] match A[int,
   * int] --> {T -> int}
   *
   * <p>The complicated case is when we have subclasses: e.g. conider two classes class A[S,T] class
   * B[X,Y] extends A[X,List[X]]
   *
   * <p>B[int,string] match A[int, List[int]] --> {} ⋀X Y. B[int,string] match A[X, Y] --> {X ->
   * int, Y -> List[int]}
   *
   * <p>The given mapping are already mapped type parameters.
   */
  abstract VariableBinding matchAgainstSupertypeIntern(
      WurstType other,
      @Nullable Element location,
      VariableBinding mapping,
      VariablePosition variablePosition);
  ...
  public VariableBinding matchTypes(
      WurstType otherType,
      @Nullable Element location,
      VariableBinding mapping,
      VariablePosition variablePosition) {
  ...
  public int hashCode() {
    throw new Error(
        "Hash code not implemented for types, because it could conflict with the custom equals operation.");
  }
  ...
  public boolean isTranslatedToInt() {
    return this instanceof WurstTypeInt
        ...
        || this
            instanceof WurstTypeBoundTypeParam; //  WurstTypeBoundTypeParam overrides this method
  }
  ...
  /**
   * checks if this type is nested inside another type or the same class for example inner classes
   * are nested inside the outer classes
   */
