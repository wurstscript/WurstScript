...
public class FunctionSignature {
  ...
  public FunctionSignature(
      HasFunctionSignature trace,
      VariableBinding mapping,
      @Nullable WurstType receiverType,
      String name,
      List<WurstType> paramTypes,
  ...
  public static FunctionSignature forFunctionDefinition(@Nullable FunctionDefinition f) {
    //		return new FunctionSignature(def.attrReceiverType(), def.attrParameterTypes(),
    // def.attrReturnTyp());
    if (f == null) {
    ...
    return new FunctionSignature(
        f,
        VariableBinding.emptyMapping().withTypeVariables(typeParams),
        f.attrReceiverType(),
        f.getName(),
        paramTypes,
        paramNames,
  ...
  public static FunctionSignature fromNameLink(FuncLink f) {
    ...
    return new FunctionSignature(
        ...
        f.getName(),
        f.getParameterTypes(),
        getParamNames(f.getDef().getParameters()),
        f.getReturnType());
  }
  ...
  public List<TypeParamDef> getDefinitionTypeVariables() {
    return trace.match(
        new HasFunctionSignature.Matcher<List<TypeParamDef>>() {
          @Override
  ...
  public ArgsMatchResult tryMatchAgainstArgs(
      List<WurstType> argTypes, List<Expr> args, Element location) {
    ...
    for (int i = 0; i < argTypes.size() && i < getMaxNumParams(); i++) {
      ...
      VariableBinding mapping2 =
          at.matchAgainstSupertype(pt, location, mapping, VariablePosition.RIGHT);
      if (mapping2 == null) {
    ...
    if (mapping.hasUnboundTypeVars()) {
      errors.add(
          new CompileError(
              location,
              "Could not infer type for type variables " + mapping.printUnboundTypeVars()));
    }
