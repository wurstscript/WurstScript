...
public class TypesHelper {
  ...
  public static boolean typeContainsTuples(ImType vt) {
    return vt instanceof ImTupleType
        ...
        || vt instanceof ImArrayTypeMulti
            && typeContainsTuples(((ImArrayTypeMulti) vt).getEntryType());
  }
  ...
  //	public static boolean checkTypeArgs(InstanceDef iDef, List<PscriptType> classParams,
  // List<PscriptType> interfaceParams) {
  ...
  //
  //		PscriptTypeInterface interfaceType = (PscriptTypeInterface)
  // iDef.getImplementedTyp().attrTyp();
  ...
  //			throw new CompileError(iDef.getSource(), "sizes do not match: " +
  // actualInterfaceTypes.size()+ " != " + interfaceParams.size());
  //		}
  //
  //		i = 0;
