...
public class AntlrJurstParseTreeTransformer {
  ...
  public AntlrJurstParseTreeTransformer(
    ...
    if (p.initializer != null) {
      ...
      elements.add(
          Ast.InitBlock(
              src,
              Ast.WStatements(
                  Ast.ExprFunctionCall(src, funcName, Ast.TypeExprList(), Ast.Arguments()))));
    }
  ...
  private Modifier annotation(WPos src, String name, String message) {
    return Ast.Annotation(
        src, Ast.Identifier(src, name), Ast.Arguments(Ast.ExprStringVal(src, message)));
  }
  ...
  private ClassSlotResult transformClassSlots(WPos src, @Nullable ClassSlotsContext slots) {
  ...
  private ConstructorDef transformConstructorDef(ConstructorDefContext c) {
    ...
    WParameters parameters = transformFormalParameters(c.formalParameters(), false);
  ...
  private WStatement transformMemberMethodCall(ExprMemberMethodContext e) {
    ...
    return transformMemberMethodCall2(source, receiver, dots, funcName, typeArgs, args);
  }
  ...
  private ExprMemberMethod transformMemberMethodCall2(
    ...
    if (dots.getType() == JurstParser.DOT) {
      return Ast.ExprMemberMethodDot(
          source, left, text(funcName), transformTypeArgs(typeArgs), transformExprs(args));
    } else {
  ...
  private ExprFunctionCall transformFunctionCall(ExprFunctionCallContext c) {
    return Ast.ExprFunctionCall(
        source(c), text(c.funcName), transformTypeArgs(c.typeArgs()), transformExprs(c.exprList()));
  }
  ...
  private Expr transformExpr(ExprContext e) {
    ...
    if (e.exprPrimary() != null) {
      ...
      return transformMemberMethodCall2(
          source, e.receiver, e.dotsCall, e.funcName, e.typeArgs(), e.exprList());
  ...
  private ExprClosure transformClosure(ExprClosureContext e) {
    ...
    for (WParameter p : parameters) {
      sparameters.add(
          Ast.WShortParameter(
              p.getSource(), p.getModifiers().copy(), p.getTyp().copy(), p.getNameId().copy()));
    }
  ...
  private FuncSig transformFuncSig(FuncSignatureContext s) {
    ...
    WParameters formalParameters = transformFormalParameters(s.formalParameters(), false);
  ...
  private TypeExpr transformTypeExpr(TypeExprContext t) throws Error {
    ...
    if (t.thistype != null) {
      ...
      return Ast.TypeExprSimple(
          source(t), scopeType, text(t.typeName), transformTypeArgs(t.typeArgs()));
  ...
  private WPos source(ParserRuleContext p) {
    return new WPos(file, lineOffsets, p.start.getStartIndex(), p.stop.getStopIndex() + 1);
  }
