...
public class ExtendedJurstLexer implements TokenSource {
  ...
  private Token nextTokenIntern() {
    ...
    if (eof_local != null) {
      return makeToken(
          JurstParser.EOF, "$EOF", eof_local.getStartIndex(), eof_local.getStopIndex());
    }
    ...
    for (; ; ) {
      ...
      if (isWurst) {
        ...
        if (token.getType() == JurstParser.PACKAGE
          ...
          token =
              makeToken(
                  JurstParser.ID, token.getText(), token.getStartIndex(), token.getStopIndex());
          assert token != null;
      ...
      if (token.getType() == JurstParser.EOF) {
        ...
        if (isWurst) {
          ...
          nextTokens.add(
              makeToken(
                  JurstParser.ENDPACKAGE,
                  "endpackage",
                  token.getStartIndex(),
                  token.getStopIndex()));
          nextTokens.add(
              makeToken(JurstParser.NL, "$NL", token.getStartIndex(), token.getStopIndex()));
        }
