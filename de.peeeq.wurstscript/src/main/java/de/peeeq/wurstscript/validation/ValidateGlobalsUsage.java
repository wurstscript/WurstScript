...
public class ValidateGlobalsUsage {
  ...
  private static void checkJassGlobals(
      JassToplevelDeclarations jassDecls, Map<NameDef, Element> usedGlobals) {
    for (JassToplevelDeclaration jassDecl : jassDecls) {
      if (jassDecl instanceof JassGlobalBlock) {
        ...
        for (GlobalVarDef glob : globals) {
          if (!glob.getSource().getFile().endsWith("common.j")
              && !glob.getSource().getFile().endsWith("blizzard.j")
