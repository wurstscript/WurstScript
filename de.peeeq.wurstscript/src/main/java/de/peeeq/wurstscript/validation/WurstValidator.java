 ...
 *
 * <p>it has visit methods for different elements in the AST and checks whether these are correct
 *
 * <p>the validation phase might not find all errors, code transformation and optimization phases
 * might detect other errors because they do a more sophisticated analysis of the program
 *
 * <p>also note that many cases are already caught by the calculation of the attributes
 */
public class WurstValidator {
  ...
  public void validate(List<CompilationUnit> toCheck) {
    try {
      ...
      prog.getErrorHandler()
          .setProgress(
              "Checking wurst types",
      ...
      if (le != null) {
        le.addError(
            "Encountered compiler bug near element "
                + Utils.printElement(le)
                + ":\n"
  ...
  private void checkUnusedImports(WPackage p) {
    ...
    for (WImport imp : p.getImports()) {
      Set<WPackage> contributedPackages =
          contributedPackages(imp.attrImportedPackage(), used, new HashSet<>());
        ...
        for (WImport imp2 : p.getImports()) {
          ...
          if (contributions.get(imp2).containsAll(impContributions)) {
            imp.addWarning(
                "The import "
                    + imp.getPackagename()
                    + " can be removed, because it is already included in "
                    + imp2.getPackagename()
                    + ".");
            break;
          }
  ...
  private Set<WPackage> contributedPackages(
      WPackage p, Set<PackageOrGlobal> used, Set<WPackage> visited) {
    if (p == null) {
  ...
  private void check(Element e) {
    try {
      if (e instanceof Annotation) checkAnnotation((Annotation) e);
      ...
      if (e instanceof ClassOrModule) checkConstructorsUnique((ClassOrModule) e);
      ...
      if (e instanceof TypeExprArray) checkCodeArrays((TypeExprArray) e);
      if (e instanceof TupleDef) checkTupleDef((TupleDef) e);
        ...
        if (f.attrIsAbstract()) {
          if (f.attrNearestStructureDef() == c) {
            ...
            loc.addError(
                "Non-abstract class "
                    + c.getName()
                    + " cannot have abstract functions like "
                    + f.getName());
      ...
      for (WParameter param : e.getParameters()) {
        ...
        if (t instanceof WurstTypeTuple) {
          ...
          if (checkTupleDefCycle(tDef, tuples)) {
            param.addError(
                "Parameter " + param.getName() + " is recursive. This is not allowed for tuples.");
            return true;
          }
        }
      }
  ...
  private void checkName(AstElementWithNameId e) {
    ...
    if (def != e && def instanceof NativeType) {
      e.addError(
          "The name '"
              + name
              + "' is already used as a native type in "
              + Utils.printPos(def.getSource()));
  ...
  private void checkConfigOverride(NameDef e) {
    ...
    if (e instanceof GlobalVarDef) {
      ...
      if (!v.attrTyp().equalsType(origVar.getTyp(), v)) {
        e.addError(
            "Configured variable must have type "
                + origVar.getTyp()
                + " but the found type is "
                + v.attrTyp()
                + ".");
        return;
      }
      ...
      if (!origVar.getDef().hasAnnotation("@configurable")) {
        e.addWarning(
            "The configured variable "
                + v.getName()
                + " is not marked with @configurable.\n"
      ...
      Collection<FuncLink> funcs =
          origPackage.getElements().lookupFuncsNoConfig(funcDef.getName(), false);
  ...
  private void checkModuleTypeUsedCorrectly(TypeExpr e, ModuleDef md) {
    ...
    if (e instanceof TypeExprSimple) {
      ...
      if (tes.getScopeType() instanceof TypeExpr) {
        ...
        if (scopeType instanceof TypeExprThis || scopeType.attrTypeDef() instanceof ModuleDef) {
  ...
  private void checkExprNull(ExprNull e) {
    if (!Utils.isJassCode(Optional.of(e)) && e.attrExpectedTyp() instanceof WurstTypeUnknown) {
      e.addError(
          "Cannot use 'null' constant here because "
  ...
  private void checkStmtSet(StmtSet s) {
    ...
    if (nameLink == null) {
      s.getUpdatedExpr()
          .addError("Could not find variable " + s.getUpdatedExpr().getVarName() + ".");
      return;
    }
  ...
  private void checkIfNoEffectAssignment(StmtSet s) {
    if (refersToSameVar(s.getUpdatedExpr(), s.getRight())) {
      s.addWarning(
          "The assignment to "
              + Utils.printElement(s.getUpdatedExpr().attrNameDef())
  ...
  private void checkAssignment(
      boolean isJassCode, Element pos, WurstType leftType, WurstType rightType) {
    ...
    if (rightType instanceof WurstTypeVoid) {
      if (pos.attrNearestPackage() instanceof WPackage) {
        ...
        if (pack != null && !pack.getName().equals("WurstREPL")) { // allow
          ...
          // Repl
          pos.addError(
              "Function or expression returns nothing. Cannot assign nothing to a variable.");
        }
      }
    }
  }
        ...
        if (!expr.attrTyp().isSubtypeOf(baseType, expr)) {
          expr.addError(
              "Expected expression of type "
                  + baseType
                  + " in array initialization, but found "
                  + expr.attrTyp());
        }
      }
    } else {
      def.addError(
          "Array initializer can only be used with array-variables, but "
              + Utils.printElement(def)
              + " has type "
              + leftType);
    }
  }
  ...
  private void checkIfRead(VarDef s) {
    ...
    if (f != null && !f.attrReadVariables().contains(s)) {
      s.addWarning(
          "The "
              + Utils.printElement(s)
              + " is never read. If intentional, prefix with \"_\" to suppress this warning.");
    }
  }
  ...
  private void visit(GlobalVarDef s) {
    ...
    if (s.attrTyp() instanceof WurstTypeArray
        && !s.attrIsStatic()
        && s.attrIsDynamicClassMember()) {
  ...
  private void checkReachability(WStatement s) {
    if (s.getParent() instanceof WStatements) {
      ...
      if (s.attrPreviousStatements().isEmpty()) {
        if (s.attrListIndex() > 0
            || !(stmts.getParent() instanceof TranslatedToImFunction
  ...
  private boolean mightBeAffectedBySwitchThatCoversAllCases(WStatement s) {
    ...
    s.attrNearestNamedScope()
        .accept(
            new Element.DefaultVisitor() {
              @Override
  ...
  private void visit(FuncDef func) {
    ...
    func.getErrorHandler()
        .setProgress(null, ProgressHelper.getValidatorPercent(visitedFunctions, functionCount));
      ...
      if (func.attrIsAbstract()) {
        ...
        if (!func.attrHasEmptyBody()) {
          func.getBody()
              .get(0)
              .addError(
                  "The abstract function " + func.getName() + " must not have any statements.");
        }
      }
    }
  ...
  private void checkCall(StmtCall call) {
    ...
    call.attrCallSignature()
        .checkSignatureCompatibility(call.attrFunctionSignature(), funcName, call);
  }
  ...
  private void visit(ExprFunctionCall stmtCall) {
    ...
    if (stmtCall.attrFuncLink() != null) {
      ...
      if (calledFunc.getDef() instanceof ExtensionFuncDef) {
        stmtCall.addError(
            "Extension function "
                + funcName
                + " must be called with an explicit receiver.\n"
                + "Try to write this."
                + funcName
                + "(...) .");
      }
    ...
    if (Utils.oneOf(funcName, "Condition", "Filter") && !stmtCall.getArgs().isEmpty()) {
      ...
      if (firstArg instanceof ExprFuncRef) {
        ...
        if (f != null) {
          if (!(f.getReturnType() instanceof WurstTypeBool)
              && !(f.getReturnType() instanceof WurstTypeVoid)) {
            firstArg.addError(
                "Functions passed to Filter or Condition must return boolean or nothing.");
          }
        }
      }
    }
  ...
  private void checkParams(
      Element where, String preMsg, List<Expr> args, List<WurstType> parameterTypes) {
  ...
  private void visit(ExprBinary expr) {
    ...
    if (def != null) {
      ...
      CallSignature callSig =
          new CallSignature(expr.getLeft(), Collections.singletonList(expr.getRight()));
    ...
    if (s.getReturnedObj() instanceof Expr) {
      ...
      if (returnType.isSubtypeOf(WurstTypeVoid.instance(), s)) {
        ...
        if (!returnedType.isSubtypeOf(returnType, s)) {
          s.addError(
              "Cannot return " + returnedType + ", expected expression of type " + returnType);
        }
      }
    } else { // empty return
      if (!returnType.isSubtypeOf(WurstTypeVoid.instance(), s)) {
  ...
  private void visit(WImport wImport) {
    ...
    if (!wImport.attrImportedPackage().getName().equals("Wurst")
        && wImport
            .attrImportedPackage()
            .getName()
            .equals(wImport.attrNearestNamedScope().getName())) {
      wImport.addError("Packages cannot import themselves");
    }
  }

  /**
   * check if the nameRef e is accessed correctly i.e. not using a dynamic variable from a static
   * context
   *
   * @param e
        ...
        if (!isUsedAsReceiverInExprMember(e)) {
          ...
          if (em.attrFuncDef() instanceof ExtensionFuncDef) {
            e.addError(
                "Reference to "
                    + e.getVarName()
                    + " can only be used for calling static methods, but not for calling extension method method '"
                    + em.getFuncName()
                    + "'.");
          }
        }
  ...
  private void checkTypeBinding(HasTypeArgs e) {
    VariableBinding mapping =
        e.match(
            new HasTypeArgs.Matcher<VariableBinding>() {
      ...
      if (tp.getTypeParamConstraints() instanceof TypeExprList) {
        ...
        if (!typ.isTranslatedToInt() && !(e instanceof ModuleUse)) {
          ...
          if (toIndexFuncs.isEmpty()) {
            e.addError(
                "Type parameters can only be bound to ints and class types, but "
                    + "not to "
                    + typ
                    + ".\n"
                    + "You can provide functions "
                    + toIndexFuncName
                    + " and "
                    + fromIndexFuncName
                    + " to use this type "
            ...
            if (fromIndex instanceof FuncDef) {
              ...
              if (!returnType.equalsType(typ, e)) {
                fromIndexF.addError(
                    "Return type must be of type " + typ + " but was " + returnType);
              }
    ...
    if (def != null && def.hasAnnotation("@deprecated")) {
      ...
      msg =
          (msg == null || msg.isEmpty())
              ? "It shouldn't be used and will be removed in the future."
              : msg;
          ...
          new HasModifier.MatcherVoid() {
            ...
            public void case_GlobalVarDef(GlobalVarDef g) {
              if (g.attrNearestClassOrModule() != null) {
                check(
                    VisibilityPrivate.class,
                    VisibilityProtected.class,
                    ModStatic.class,
                    ModConstant.class,
                    Annotation.class);
              } else {
          ...
          if (d.getSuperConstructorCall() instanceof NoSuperConstructorCall
            ...
            c.addError(
                "The extended class <"
                    ...
                    + "> does not expose a no-arg constructor. "
      ...
      if (constr.attrNearestClassDef().attrIsAbstract()) {
        e.addError(
            "Cannot create an instance of the abstract class "
                + constr.attrNearestClassDef().getName());
        return;
      }
      ...
      for (int i = 0; i < switchExprs.size(); i++) {
        ...
        for (int j = 0; j < i; j++) {
          ...
          if (ei.structuralEquals(ej)) {
            ei.addError(
                "The case "
                    + Utils.prettyPrint(ei)
                    + " is already handled in line "
                    + ej.attrSource().getLine());
            return;
          }
        }
      }
  ...
  private void checkCodeArrays(TypeExprArray e) {
    if (e.getBase() instanceof TypeExprSimple) {
      ...
      if (base.getTypeName().equals("code")) {
        e.addError(
            "Code arrays are not supported. Try using an array of triggers or conditionfuncs.");
      }
    }
  }
   ...
   * checks if func1 can override func2
   *
   * <p>Returns null if yes and an error message if not.
   */
  public static String checkOverride(FuncLink func1, FuncLink func2, boolean allowStaticOverride) {
    ...
    if (paramCount1 != paramCount2) {
      return Utils.printElement(func2.getDef())
          + " takes "
          + paramCount2
          + " parameters, but there are only "
          + paramCount1
          + " parameters here.";
    }
            ...
            f1.getDef()
                .addError(
                    "Function already defined : "
                        + Utils.printElementWithSource(Optional.of(f2.getDef())));
            f2.getDef()
                .addError(
                    "Function already defined : "
                        + Utils.printElementWithSource(Optional.of(f1.getDef())));
          }
        }
      }
    }
  ...
  private void checkVarDef(VarDef v) {
    ...
    if (vtype instanceof WurstTypeCode && v.attrIsDynamicClassMember()) {
      v.addError(
          "Code members not allowed as dynamic class members (variable "
              + v.getName()
              + ")\n"
    ...
    if (v instanceof GlobalOrLocalVarDef) {
      ...
      if (g.attrIsConstant()
          && g.getInitialExpr() instanceof NoExpr
    ...
    if (vtype instanceof WurstTypeNull) {
      v.addError(
          "Initial value of variable " + v.getName() + " is 'null'. Specify a concrete type.");
    }
  }
  ...
  private void checkLocalShadowing(LocalVarDef v) {
    ...
    if (shadowed != null) {
      if (shadowed.getDef() instanceof LocalVarDef) {
        v.addError(
            "Variable " + v.getName() + " hides an other local variable with the same name.");
