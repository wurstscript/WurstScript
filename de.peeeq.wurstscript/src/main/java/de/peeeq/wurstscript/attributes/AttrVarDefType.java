...
public class AttrVarDefType {
  ...
  public static WurstType getParameterTypeFromClosureType(
      WShortParameter p, int paramIndex, WurstType expectedTyp, boolean addError) {
      ...
      if (addError) {
        p.addError(
            "Could not infer type for parameter "
                + p.getName()
                + ". "
                + "The target type could not be uniquely determined for expected type "
                + expectedTyp
                + ".");
      }
      return WurstTypeInfer.instance();
      ...
      if (addError) {
        p.addError(
            "Could not infer type for parameter "
                ...
                + "Closure type "
                + expectedTyp
                + " does not take so many parameters.");
      }
      return WurstTypeInfer.instance();
    }
  ...
  private static WurstType defaultCase(GlobalOrLocalVarDef v) {
    ...
    if (typ instanceof TypeExpr) {
      ...
      if (initialExpr instanceof Expr) {
        ...
        if (values.isEmpty()) {
          v.addError(
              "Could not infer the type of variable '"
                  + v.getName()
        ...
        if (valueType instanceof WurstTypeIntLiteral) {
          ...
          v.addError(
              "Array parameters are not permitted. Remember that initialized arrays do not have an identity nor length.");
          return new WurstTypeArray(WurstTypeUnknown.instance());
        }
        ...
        v.addError(
            "Could not infer the type of variable '"
                + v.getName()
                + "' because it does not have an initial expression.\n"
                + "Fix this error by providing a type (e.g. 'int "
                + v.getName()
                + "' or 'string "
                + v.getName()
                + "').");
