...
/**
 * This attribute calculates the expected type for an expression for example if you have:
 *
 * <p>function foo(A a) function bar() returns B
 *
 * <p>and call it with foo(bar()), then the expected type of the expected type of the expression
 * bar() will be A and the actual type (see attrExprType) will be B.
 */
public class AttrExprExpectedType {
  ...
  public static @NonNull WurstType calculate(Expr expr) {
    try {
      ...
      WLogger.info(
          "Something went wrong while computing the expected type for "
  ...
  private static WurstType expectedTypeAfterOverloading(
      Expr expr, Arguments args, StmtCall stmtCall) {
