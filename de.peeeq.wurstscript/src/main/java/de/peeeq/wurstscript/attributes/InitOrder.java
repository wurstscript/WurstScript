...
public class InitOrder {
  ...
  private static List<String> toStringArray(List<WPackage> importChain) {
    return importChain.stream().map(WPackage::getName).collect(Collectors.toList());
  }
  ...
  private static void collectImportedPackages(
      List<WPackage> callStack, WPackage p, Collection<WPackage> result) {
    callStack.add(p);
