...
public class AttrExprType {
   ...
   */
  public static WurstType caclulateThistype(
      Element term, boolean dynamic, @Nullable String searchedTerm) {
    ...
    if (c != null) {
      return c.match(
          new NamedScope.Matcher<WurstType>() {
      ...
      case NOTEQ:
        {
          ...
          if (Utils.isJassCode(Optional.of(term))) {
            if (leftType.isSubtypeOf(WurstTypeReal.instance(), term)
                || leftType.isSubtypeOf(WurstTypeInt.instance(), term)) {
              if (rightType.isSubtypeOf(WurstTypeReal.instance(), term)
            ...
            if (leftType.isSubtypeOf(WurstTypeNull.instance(), term)
                    && rightType.isSubtypeOf(WurstTypeInt.instance(), term)
                || leftType.isSubtypeOf(WurstTypeInt.instance(), term)
                    && rightType.isSubtypeOf(WurstTypeNull.instance(), term)) {
              // in jass code it is allowed to compare an integer with 'null'. wat?
              return WurstTypeBool.instance();
            }

            if (leftType.isSubtypeOf(
                    WurstNativeType.instance("agent", WurstTypeNull.instance()), term)
  ...
  private static WurstType handleOperatorOverloading(ExprBinary term) {
    ...
    if (def == null) {
      term.addError(
          "No operator overloading function for operator "
              + term.getOp()
              + " was found for operands "
              + leftType
              + " and "
              + rightType
              + ". The overloading function has to be named: "
              + term.getOp().getOverloadingFuncName());
      return WurstTypeUnknown.instance();
    }
    return def.getReturnType();
  }

  private static WurstType requireEqualTypes(
      ExprBinary term, WurstTypeBool requiredType, WurstTypeBool resultType) {
    final WurstType leftType = term.getLeft().attrTyp();
    final WurstType rightType = term.getRight().attrTyp();
  ...
  private static boolean bothTypesRealOrInt(ExprBinary term) {
    WurstType leftType = term.getLeft().attrTyp();
    WurstType rightType = term.getRight().attrTyp();
    return ((leftType.isSubtypeOf(WurstTypeInt.instance(), term)
            || leftType.isSubtypeOf(WurstTypeReal.instance(), term))
        && (rightType.isSubtypeOf(WurstTypeInt.instance(), term)
            || rightType.isSubtypeOf(WurstTypeReal.instance(), term)));
  }
  ...
  public static WurstType calculate(ExprMemberVarDot term) {
    ...
    if (varDef.getDef().attrIsStatic() && !term.getLeft().attrTyp().isStaticRef()) {
      term.addError(
          "Cannot access static variable " + term.getVarName() + " via a dynamic reference.");
    }
  ...
  public static WurstType calculate(ExprMemberArrayVarDot term) {
    ...
    if (varDef.getDef().attrIsStatic() && !term.getLeft().attrTyp().isStaticRef()) {
      term.addError(
          "Cannot access static array variable " + term.getVarName() + " via a dynamic reference.");
    }
  ...
  private static void checkCastOrInstanceOf(
      Element e, WurstType exprType, WurstType targetType, String msgPre) {
  ...
  public static WurstType calculate(ExprIfElse e) {
    ...
    if (tf.isSubtypeOf(WurstTypeVoid.instance(), e)) {
      e.getIfTrue()
          .addError(
              "Conditional expression must return a value, but result type of else-expression is void.");
    }
