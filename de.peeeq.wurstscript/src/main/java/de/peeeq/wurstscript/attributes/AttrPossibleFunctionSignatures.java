...
public class AttrPossibleFunctionSignatures {
  ...
  public static ImmutableCollection<FunctionSignature> calculate(FunctionCall fc) {
    ...
    for (FuncLink f : fs) {
      ...
      OptExpr implicitParameterOpt =
          AttrImplicitParameter.getFunctionCallImplicitParameter(fc, f, false);
      if (implicitParameterOpt instanceof Expr) {
        Expr expr = (Expr) implicitParameterOpt;
        VariableBinding mapping =
            expr.attrTyp()
                .matchAgainstSupertype(
                    sig.getReceiverType(), fc, sig.getMapping(), VariablePosition.RIGHT);
        if (mapping == null) {
  ...
  private static ImmutableCollection<FunctionSignature> findBestSignature(
      StmtCall fc, ImmutableCollection<FunctionSignature> res) {
    ...
    if (res2.isEmpty()) {
      ...
      ImmutableList<ArgsMatchResult> match3 =
          res.stream()
      ...
      if (match3.isEmpty()) {
        ...
        ArgsMatchResult min =
            Collections.min(match3, Comparator.comparing(ArgsMatchResult::getBadness));
  ...
  public static ImmutableCollection<FunctionSignature> calculate(ExprNewObject fc) {
    ...
    for (ConstructorDef f : constructors) {
      ...
      FunctionSignature sig =
          new FunctionSignature(f, mapping, null, "construct", paramTypes, pNames, returnType);
