...
import de.peeeq.wurstscript.attributes.funcs.TypeArgs;
...
public class AttrPossibleFunctionSignatures {
    ...
    public static ImmutableCollection<FuncSig> calculate(FunctionCall fc) {
        ImmutableCollection<FuncSig> fs = fc.attrPossibleFuncDefs();
        ImmutableCollection.Builder<FuncSig> resultBuilder = ImmutableList.builder();
        for (FuncSig sig : fs) {
            sig = TypeArgs.getTypeArgs(sig, fc);
            resultBuilder.add(sig);
        }
        ImmutableCollection.Builder<FuncSig> resultBuilder2 = ImmutableList.builder();
        ImmutableCollection<FuncSig> res = resultBuilder.build();
        for (FuncSig sig : res) {
            if (paramTypesCanMatch(sig.getParamTypes(), AttrFuncDef.argumentTypes(fc), fc)) {
                resultBuilder2.add(sig);
            }
        }
        ImmutableCollection<FuncSig> res2 = resultBuilder2.build();
        if (res2.isEmpty()) {
            return res;
        } else {
            return res2;
    ...
    public static ImmutableCollection<FuncSig> calculate(ExprNewObject fc) {
        ...
        return ImmutableList.of(new FuncSig(f, ImmutableList.of(), paramTypes, pNames, returnType, ImmutableList.of()));
    }
