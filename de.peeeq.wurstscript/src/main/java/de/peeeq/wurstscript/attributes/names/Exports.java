...
public class Exports {
  ...
  private static void addExportedNameLinks(
      Builder<String, DefLink> result, WPackage p, Set<WPackage> alreadyImported) {
    ...
    for (WImport imp2 : p.getImports()) {
      if (imp2.getIsPublic()) {
        @Nullable WPackage imported = imp2.attrImportedPackage();
        if (imported != null) {
  ...
  private static void addExportedTypeNameLinks(
      Builder<String, TypeLink> result, WPackage p, Set<WPackage> alreadyImported) {
    if (alreadyImported.contains(p)) {
      return;
    }
    alreadyImported.add(p);
