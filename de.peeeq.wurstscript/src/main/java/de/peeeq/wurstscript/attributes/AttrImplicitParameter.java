...
public class AttrImplicitParameter {
  ...
  static OptExpr getFunctionCallImplicitParameter(
      FunctionCall e, FuncLink calledFunc, boolean showError) {
    ...
    if (calledFunc.getDef().attrIsDynamicClassMember()) {
      ...
      if (e.attrIsDynamicContext()) {
        ...
        if (showError && !t.attrTyp().isSubtypeOf(calledFunc.getReceiverType(), e)) {
          e.addError(
              "Cannot access dynamic function "
                  + e.getFuncName()
                  + " from context of type "
                  + t.attrTyp()
                  + ".");
        }
        return t;
      ...
      if (varDef.attrIsDynamicClassMember()) {
        ...
        if (e.attrIsDynamicContext()) {
          ...
          if (!def.receiverCompatibleWith(thisType, e)) {
            e.addError(
                "Cannot access dynamic variable "
                    + varDef.getName()
                    + " from context of type "
                    + thisType
                    + ".");
          ...
          e.addError(
              "Cannot access dynamic variable " + varDef.getName() + " from static context.");
          return Ast.NoExpr();
        }
      }
    }
