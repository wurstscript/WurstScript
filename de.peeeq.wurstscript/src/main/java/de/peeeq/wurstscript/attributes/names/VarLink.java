...
public class VarLink extends DefLink {
  ...
  public static VarLink create(TypeDef def, WScope definedIn) {
    ...
    List<TypeParamDef> typeParams =
        Streams.concat(typeParams(definedIn), typeParams(def)).collect(Collectors.toList());
  ...
  public VarLink withTypeArgBinding(Element context, VariableBinding binding) {
    ...
    if (changed) {
      List<TypeParamDef> newTypeParams =
          getTypeParams().stream().filter(binding::contains).collect(Collectors.toList());
      return new VarLink(
          getVisibility(), getDefinedIn(), newTypeParams, newReceiverType, def, newType);
    } else {
      return this;
    }
  }
  ...
  public DefLink withGenericTypeParams(List<TypeParamDef> typeParams) {
    ...
    return new VarLink(
        getVisibility(), getDefinedIn(), newTypeParams, getReceiverType(), def, type);
  }
  ...
  public VarLink withDef(NameDef def) {
    return new VarLink(
        getVisibility(), getDefinedIn(), getTypeParams(), getReceiverType(), def, type);
  }
