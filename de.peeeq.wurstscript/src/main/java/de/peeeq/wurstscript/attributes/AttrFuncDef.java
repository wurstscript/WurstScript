...
public class AttrFuncDef {
  ...
  private static @Nullable FuncLink getExtensionFunction(Expr left, Expr right, WurstOperator op) {
    ...
    return searchMemberFunc(
        left, left.attrTyp(), funcName, Collections.singletonList(right.attrTyp()));
  }

  /**
   * checks if operator is a native operator like for 1+2 TODO also check which operator is used?
   *
   * @param op
   * @param term
   */
  private static boolean nativeOperator(
      WurstOperator op, WurstType leftType, WurstType rightType, Element term) {
    return
    // numeric
    ((leftType.isSubtypeOf(WurstTypeInt.instance(), term)
                || leftType.isSubtypeOf(WurstTypeReal.instance(), term))
            && (rightType.isSubtypeOf(WurstTypeInt.instance(), term)
                || rightType.isSubtypeOf(WurstTypeReal.instance(), term)))
        // strings
        || (op == WurstOperator.PLUS
            && leftType instanceof WurstTypeString
            && rightType instanceof WurstTypeString);
  }
  ...
  public static List<WurstType> argumentTypes(StmtCall node) {
    ...
    for (Expr arg : node.getArgs()) {
      ...
      if (arg instanceof ExprClosure) {
        ...
        if (closure.getShortParameters().stream()
            .allMatch(p -> p.getTypOpt() instanceof TypeExpr)) {
          argType = arg.attrTyp();
        } else {
          ...
          for (WShortParameter p : closure.getShortParameters()) {
            if (p.getTypOpt() instanceof TypeExpr) {
              ...
              WurstType pt =
                  AttrVarDefType.getParameterTypeFromClosureType(p, pIndex, expected, false);
              paramTypes.add(pt);
          ...
          if (hasInferredType) {
            // if there are unknown parameter types, use an approximated function type for
            // overloading resolution
            WurstType resultType = WurstTypeInfer.instance();
            argType = new WurstTypeClosure(paramTypes, resultType);
          } else {
            // if there are no unknown types for the argument, then it should be safe to directly
            // calculate the type
            argType = arg.attrTyp();
          }
        }
  ...
  private static FuncLink searchFunction(
      String funcName, @Nullable FuncRef node, List<WurstType> argumentTypes) {
    if (node == null) {
      return null;
    }
  ...
  static ImmutableList<FuncLink> filterAnnotation(
      FuncRef node, ImmutableCollection<FuncLink> funcs1) {
    ...
    ImmutableList<FuncLink> res = funcs1.stream().filter(filter).collect(Utils.toImmutableList());
    if (res.isEmpty()) {
  ...
  private static List<FuncLink> ignoreWithIfNotDefinedAnnotation(
      FuncRef node, List<FuncLink> funcs) {
    return funcs.stream()
  ...
  private static @Nullable FuncLink searchMemberFunc(
      Expr node, WurstType leftType, String funcName, List<WurstType> argumentTypes) {
  ...
  private static List<FuncLink> filterByParameters(
      Element node, List<WurstType> argumentTypes, List<FuncLink> funcs) {
  ...
  private static List<FuncLink> filterByParameterTypes(
    ...
    if (funcs4.size() == 0) {
      ...
      // if some argument type could not be determined, we don't want errors here, just take the
      // first one
      return ImmutableList.of(Utils.getFirst(funcs4));
    }
    return funcs4;
  ...
  private static List<FuncLink> filterByParamaeterNumber(
      List<WurstType> argumentTypes, List<FuncLink> funcs2) {
    ...
    for (FuncLink f : funcs2) {
      if (f.getParameterTypes().size() == argumentTypes.size()
          || (f.getParameterTypes().size() == 1
              && f.getParameterTypes().get(0) instanceof WurstTypeVararg)) {
        funcs3.add(f);
      }
    }
    if (funcs3.size() == 0) {
  ...
  private static List<FuncLink> filterInvisible(
      String funcName, Element node, Collection<FuncLink> funcs1) {
  ...
  private static List<FuncLink> filterByReceiverType(
      Element node, String funcName, List<FuncLink> funcs2) {
    List<FuncLink> funcs3 = Lists.newArrayListWithCapacity(funcs2.size());
    for (FuncLink f : funcs2) {
  ...
  public static FuncLink calculate(Annotation node) {
    List<WurstType> argumentTypes =
        node.getArgs().stream().map(Expr::attrTyp).collect(Collectors.toList());
    ...
    if (def != null && !def.hasAnnotation("@annotation")) {
      node.addWarning(
          "The function "
              + def.getName()
              + " must be annotated with @annotation to be usable as an annotation.");
    }
    return result;
  }
}
