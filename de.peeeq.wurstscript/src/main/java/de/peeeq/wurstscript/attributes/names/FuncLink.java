...
public class FuncLink extends DefLink {
  ...
  public static FuncLink create(FunctionDefinition func, WScope definedIn) {
    ...
    List<String> lParameterNames =
        func.getParameters().stream().map(WParameter::getName).collect(Collectors.toList());
    List<WurstType> lParameterTypes =
        func.getParameters().stream().map(WParameter::attrTyp).collect(Collectors.toList());
    ...
    if (func instanceof AstElementWithTypeParameters) {
      mapping =
          mapping.withTypeVariables(((AstElementWithTypeParameters) func).getTypeParameters());
    }
    return new FuncLink(
        visibiliy,
        definedIn,
  ...
  public String toString() {
    ...
    if (!typeParams.isEmpty()) {
      result.append("<");
      result.append(
          typeParams.stream().map(TypeParamDef::getName).collect(Collectors.joining(", ")));
      result.append(">");
    }
    result.append("(");
  ...
  public FuncLink withTypeArgBinding(Element context, VariableBinding binding) {
    ...
    if (changed) {
      ...
      List<TypeParamDef> newTypeParams =
          getTypeParams().stream().filter(tp -> !binding.contains(tp)).collect(Collectors.toList());
      return new FuncLink(
          getVisibility(),
          getDefinedIn(),
          newTypeParams,
