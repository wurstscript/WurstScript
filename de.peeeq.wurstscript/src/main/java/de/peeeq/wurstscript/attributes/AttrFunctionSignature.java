...
public class AttrFunctionSignature {
  ...
  private static FunctionSignature filterSigs(
    ...
    if (argTypes.stream().noneMatch(t -> t instanceof WurstTypeUnknown)) {
      ...
      location.addError(
          "Call to " + name(location) + " is ambiguous, alternatives are: " + alternatives);
    }
    return candidates.get(0);
  }

  private static List<FunctionSignature> filterByIfNotDefinedAnnotation(
  ...
  private static boolean paramTypesMatch(
      FunctionSignature sig, List<WurstType> argTypes, Element location) {
    return paramTypesMatch(sig.getParamTypes(), argTypes, location);
  }

  private static boolean paramTypesMatch(
      List<WurstType> paramTypes, List<WurstType> argTypes, Element location) {
