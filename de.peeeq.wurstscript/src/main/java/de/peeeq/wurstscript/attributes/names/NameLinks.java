...
public class NameLinks {
  ...
  @NotNull
  private static Map<String, Map<FuncLink, OverrideCheckResult>> initOverrideMap(
    ...
    for (DefLink link : result.values()) {
      if (link instanceof FuncLink) {
        Map<FuncLink, OverrideCheckResult> map =
            overrideCheckResults.computeIfAbsent(link.getName(), s -> new HashMap<>());
  ...
  private static void reportOverrideErrors(
      Map<String, Map<FuncLink, OverrideCheckResult>> overrideCheckResults) {
    ...
    for (Map<FuncLink, OverrideCheckResult> map : overrideCheckResults.values()) {
      for (Entry<FuncLink, OverrideCheckResult> e : map.entrySet()) {
        ...
        if (f.attrIsOverride() && !check.doesOverride) {
          StringBuilder msg =
              new StringBuilder("Function " + f.getName() + " does not override anything.");
          ...
          if (check.requiresOverrideMod) {
            ...
            f.addWarning(
                "Function " + f.getName() + " should be marked with the 'override' modifier.");
          }
        }
      }
    }
  ...
  private static void addNamesFromExtendedInterfaces(
      Multimap<String, DefLink> result,
      WurstTypeInterface iType,
      Map<String, Map<FuncLink, OverrideCheckResult>> overrideCheckResults) {
  ...
  private static void addNamesFromImplementedInterfaces(
      Multimap<String, DefLink> result,
      WurstTypeClass classDef,
      Map<String, Map<FuncLink, OverrideCheckResult>> overrideCheckResults) {
  ...
  private static void addNamesFormSuperClass(
      Multimap<String, DefLink> result,
      WurstTypeClass c,
      Map<String, Map<FuncLink, OverrideCheckResult>> overrideCheckResults) {
  ...
  private static void addNewNameLinks(
      Multimap<String, DefLink> result,
      Map<String, Map<FuncLink, OverrideCheckResult>> overrideCheckResults,
      ImmutableMultimap<String, DefLink> newNameLinks,
    ...
    for (Entry<String, DefLink> e : newNameLinks.entries()) {
      ...
      if (def instanceof FuncLink) {
        ...
        Map<FuncLink, OverrideCheckResult> otherFuncs =
            overrideCheckResults.getOrDefault(name, Collections.emptyMap());
  ...
  public static ImmutableMultimap<String, DefLink> calculate(
      @SuppressWarnings("unused") NativeFunc nativeFunc) {
    ImmutableMultimap.Builder<String, DefLink> result = ImmutableSetMultimap.builder();
    return result.build();
  }
  ...
  private static void addNameDefDefLink(
      Builder<String, DefLink> result, NameDef def, WScope scope) {
    addNameDefDefLink(l -> result.put(l.getName(), l), def, scope);
  }

  private static void addNameDefDefLink(
      Multimap<String, DefLink> result, NameDef def, WScope scope) {
    addNameDefDefLink(l -> result.put(l.getName(), l), def, scope);
  }

  private static void addNamesFromUsedModuleInstantiations(
      ClassOrModuleOrModuleInstanciation c,
      Multimap<String, DefLink> result,
      Map<String, Map<FuncLink, OverrideCheckResult>> overrideCheckResults) {
  ...
  private static void addDefinedNames(
      Multimap<String, DefLink> result, ClassOrModuleOrModuleInstanciation c) {
  ...
  private static void addDefinedNames(
      Builder<String, DefLink> result, WScope definedIn, List<? extends NameDef> slots) {
    for (NameDef n : slots) {
  ...
  private static void addDefinedNames(
      Multimap<String, DefLink> result, WScope definedIn, List<? extends NameDef> slots) {
    for (NameDef n : slots) {
      addNameDefDefLink(result, n, definedIn);
    }
  }

  public static void addHidingPrivate(
      Builder<String, DefLink> result,
      Multimap<String, DefLink> adding,
      List<TypeParamDef> typeParams) {
  ...
  public static void addHidingPrivate(
      Multimap<String, DefLink> result, Multimap<String, DefLink> adding) {
    for (Entry<String, DefLink> e : adding.entries()) {
      if (e.getValue().getVisibility() == Visibility.LOCAL) {
        continue;
      }
  ...
  public static void addHidingPrivateAndProtected(
      ImmutableMultimap.Builder<String, DefLink> r, Multimap<String, ? extends DefLink> adding) {
  ...
  public static void addTypesHidingPrivateAndProtected(
      ImmutableMultimap.Builder<String, TypeLink> r, Multimap<String, TypeLink> adding) {
