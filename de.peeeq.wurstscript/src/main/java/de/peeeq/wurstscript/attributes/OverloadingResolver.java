...
public abstract class OverloadingResolver<F extends Element, C> {
  ...
  Optional<F> resolve(Iterable<F> alternativeFunctions, C caller) {
    ...
    for (F f : alternativeFunctions) {
      ...
      for (int i = 0; i < getArgumentCount(caller) && i < getParameterCount(f); i++) {
        if (getArgumentType(caller, i) instanceof WurstTypeTypeParam
          ...
          hints.add(
              "Expected "
                  + getParameterType(f, i)
                  + " as parameter "
                  + i
                  + " ,but found "
                  + getArgumentType(caller, i)
                  + ".");
          continue;
        }
        matches++;
    ...
    List<F> funcs =
        Ordering.natural().reverse().onResultOf(numMatches::get).sortedCopy(alternativeFunctions);
    ...
    return Optional.of(Ordering.natural().onResultOf(this::getParameterCount).max(funcs));
  }

  public static @Nullable ConstructorDef resolveExprNew(
      List<ConstructorDef> constructors, final ExprNewObject node) {
  ...
  public static @Nullable ConstructorDef resolveSuperCall(
      List<ConstructorDef> constructors, final ConstructorDef node) {
    return new OverloadingResolver<ConstructorDef, ConstructorDef>() {
      ...
      int getArgumentCount(ConstructorDef c) {
        return c.getSuperConstructorCall()
            .match(
                new SuperConstructorCall.Matcher<Integer>() {
                  @Override
