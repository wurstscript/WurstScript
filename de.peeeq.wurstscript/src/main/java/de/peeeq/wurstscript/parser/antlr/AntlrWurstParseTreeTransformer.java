...
public class AntlrWurstParseTreeTransformer {
  ...
  public AntlrWurstParseTreeTransformer(
      String file, ErrorHandler cuErrorHandler, LineOffsets lineOffsets) {
    this.file = file;
    ...
    try {
      for (TopLevelDeclarationContext decl : cu.decls) {
        ...
        if (decl.jassTopLevelDeclaration() != null) {
          jassDecls.add(transformJassToplevelDecl(decl.jassTopLevelDeclaration()));
  ...
  private JassToplevelDeclaration transformJassToplevelDecl(JassTopLevelDeclarationContext d) {
  ...
  private JassToplevelDeclaration transformJassNativeDecl(JassNativeDeclContext n) {
    Modifiers modifiers = Ast.Modifiers();
    FuncSig sig = transformFuncSig(n.jassFuncSignature());
    return Ast.NativeFunc(source(n), modifiers, sig.name, sig.formalParameters, sig.returnType);
  }

  private JassToplevelDeclaration transformJassGlobalsBlock(JassGlobalsBlockContext g) {
    ...
    for (JassGlobalDeclContext v : g.jassGlobalDecl()) {
      ...
      result.add(Ast.GlobalVarDef(source(v), modifiers, optTyp, name, initialExpr));
    }
    return result;
  }
  ...
  private JassToplevelDeclaration transformJassFuncDef(JassFuncDefContext f) {
    ...
    return Ast.FuncDef(
        source(f), modifiers, sig.name, sig.typeParams, sig.formalParameters, sig.returnType, body);
  }
  ...
  private WStatements transformJassLocals(List<JassLocalContext> jassLocals) {
    ...
    for (JassLocalContext l : jassLocals) {
      ...
      result.add(Ast.LocalVarDef(source(l), modifiers, optTyp, name, initialExpr));
    }
    return result;
  }
  ...
  private WEntity transformExtensionFuncDef(ExtensionFuncDefContext f) {
    ...
    return Ast.ExtensionFuncDef(
        src,
        modifiers,
        extendedType,
        sig.name,
        sig.typeParams,
        sig.formalParameters,
        sig.returnType,
        body);
  }
  ...
  private Modifier transformModifier(ModifierContext m) {
    ...
    if (m.annotation() != null) {
      return Ast.Annotation(
          src,
          Ast.Identifier(
              source(m.annotation().name),
              m.annotation().name.getText().substring(1).toLowerCase()),
  ...
  private WEntity transformInterfaceDef(InterfaceDefContext i) {
    ...
    return Ast.InterfaceDef(
        ...
        slots.vars,
        slots.constructors,
        slots.moduleInstanciations,
        slots.moduleUses,
        slots.onDestroy);
  }

  private ClassSlotResult transformClassSlots(WPos src, ClassSlotsContext slots) {
  ...
  private OnDestroyDef transformOndestroyDef(OndestroyDefContext o) {
    return Ast.OnDestroyDef(source(o), transformStatements(o.statementsBlock()));
  }

  private ClassSlot transformModuleUse(ModuleUseContext u) {
    return Ast.ModuleUse(source(u), text(u.moduleName), transformTypeArgs(u.typeArgs()));
  }
  ...
  private SuperConstructorCall transformSuperCall(SuperCallContext sc) {
    ...
    return Ast.SomeSuperConstructorCall(
        source(sc), source(sc.superKeyword), transformExprs(sc.superArgs));
  }
  ...
  private NativeType transformNativeType(NativeTypeContext n) {
    ...
    if (n.extended != null) {
      extended =
          Ast.TypeExprSimple(
              source(n.extended), Ast.NoTypeExpr(), rawText(n.extended), Ast.TypeExprList());
    } else {
      extended = Ast.NoTypeExpr();
    }
    return Ast.NativeType(source(n), Ast.Modifiers(), text(n.name), extended);
  }
  ...
  private InitBlock transformInit(InitBlockContext i) {
    return Ast.InitBlock(source(i), transformStatements(i.statementsBlock()));
  }
  ...
  private WStatement transformStatement(StatementContext s) {
    ...
    if (s.externalLambda() != null) {
      ...
      cuErrorHandler.sendError(
          new CompileError(
              source(s.externalLambda()),
  ...
  private Expr transformExternalLambda(ExternalLambdaContext el) {
    ...
    return Ast.ExprClosure(
        ...
        source(el.arrow),
        closureParams,
        Ast.ExprStatementsBlock(
            source(el.statementsBlock()), transformStatements(el.statementsBlock())));
  }
    ...
    if (s.stmtIf() != null) {
      ...
      if (e instanceof WStatement) {
        ...
        cuErrorHandler.sendError(
            new CompileError(
                source(s),
                Utils.printElement(e) + " cannot be used here. A full statement is required."));
        return Ast.StmtErr(source(s));
      }
  ...
  private WStatement transformSwitch(StmtSwitchContext s) {
    ...
    if (s.switchDefaultCase() != null) {
      switchDefault =
          Ast.SwitchDefaultCaseStatements(
              source(s.switchDefaultCase()),
  ...
  private WStatement transformWhile(StmtWhileContext s) {
    return Ast.StmtWhile(
        source(s), transformExpr(s.cond), transformStatements(s.statementsBlock()));
  }
  ...
  private WStatement transformStmtSet(StmtSetContext s) {
    ...
    if (s.assignOp != null) {
      ...
      if (op != null) {
        right = Ast.ExprBinary(src, (Expr) updatedExpr.copy(), op, right);
      }
      return Ast.StmtSet(src, updatedExpr, right);
    } else if (s.incOp != null) {
      Expr right =
          Ast.ExprBinary(
              src, (Expr) updatedExpr.copy(), WurstOperator.PLUS, Ast.ExprIntVal(src, "1"));
      return Ast.StmtSet(src, updatedExpr, right);
    } else if (s.decOp != null) {
      Expr right =
          Ast.ExprBinary(
              src, (Expr) updatedExpr.copy(), WurstOperator.MINUS, Ast.ExprIntVal(src, "1"));
      return Ast.StmtSet(src, updatedExpr, right);
    }
  ...
  private NameRef transformExprVarAccess(ExprVarAccessContext e) {
    if (e.indexes() == null) {
      ...
      return Ast.ExprVarArrayAccess(source(e), text(e.varname), transformIndexes(e.indexes()));
    }
  }

  private NameRef transformExprMemberVar(ExprMemberVarContext e) {
    return transformExprMemberVarAccess2(source(e), e.expr(), e.dots, e.varname, e.indexes());
  }

  private NameRef transformExprMemberVarAccess2(
      WPos source,
      ExprContext e_expr,
      Token e_dots,
      Token e_varname,
  ...
  private ExprMemberMethod transformMemberMethodCall2(
      WPos source,
      ExprContext receiver,
      Token dots,
      Token funcName,
      TypeArgsContext typeArgs,
      ArgumentListContext args) {
    Expr left = transformExpr(receiver);
    if (dots.getType() == WurstParser.DOT) {
      return Ast.ExprMemberMethodDot(
          source, left, text(funcName), transformTypeArgs(typeArgs), transformArgumentList(args));
    } else {
      return Ast.ExprMemberMethodDotDot(
          source, left, text(funcName), transformTypeArgs(typeArgs), transformArgumentList(args));
    }
  }
  ...
  private ExprFunctionCall transformFunctionCall(ExprFunctionCallContext c) {
    return Ast.ExprFunctionCall(
        source(c),
        text(c.funcName),
        transformTypeArgs(c.typeArgs()),
        transformArgumentList(c.argumentList()));
  }
  ...
  private WStatement transformIf(StmtIfContext i) {
    ...
    if (i.elseStatements() != null) {
      if (i.elseStatements().stmtIf() != null) {
        elseBlock = Ast.WStatements(transformIf(i.elseStatements().stmtIf()));
      } else {
        elseBlock = transformStatements(i.elseStatements().statementsBlock());
      }
    } else {
    ...
    try {
      ...
      if (e.exprPrimary() != null) {
        ...
        return Ast.ExprBinary(
            source, transformExpr(e.left), transformOp(e.op), transformExpr(e.right));
        ...
        return transformMemberMethodCall2(
            source, e.receiver, e.dotsCall, e.funcName, e.typeArgs(), e.argumentList());
      } else if (e.instaneofType != null) {
        return Ast.ExprInstanceOf(
            source, transformTypeExpr(e.instaneofType), transformExpr(e.left));
      } else if (e.cond != null) {
        return Ast.ExprIfElse(
            source,
            transformExpr(e.cond),
            transformExpr(e.ifTrueExpr),
            transformExpr(e.ifFalseExpr));
      }
  ...
  private ExprStatementsBlock transformExprStatementsBlock(ExprStatementsBlockContext e) {
  ...
  private ExprClosure transformClosure(ExprClosureContext e) {
    WShortParameters parameters = transformShortFormalParameters(e.shortFormalParameters());
    Expr implementation;
    if (e.expr() != null) {
      implementation = transformExpr(e.expr());
    } else if (e.skip != null) {
      implementation =
          Ast.ExprStatementsBlock(source(e.skip), Ast.WStatements(Ast.StmtSkip(source(e.skip))));
    } else {
  ...
  private FuncSig transformFuncSig(FuncSignatureContext s) {
    ...
    return new FuncSig(text(s.name), typeParams, formalParameters, returnType);
  }
  ...
  private TypeExpr transformTypeExpr(TypeExprContext t) throws Error {
    ...
    if (t.thistype != null) {
      ...
      return Ast.TypeExprArray(source(t), (TypeExpr) scopeType, transformOptionalExpr(t.arraySize));
    } else {
  ...
  private WShortParameters transformShortFormalParameters(ShortFormalParametersContext ps) {
    if (ps.singleParam != null) {
      return Ast.WShortParameters(
          Ast.WShortParameter(
              source(ps.singleParam).artificial(),
  ...
  private WParameter transformFormalParameter(FormalParameterContext p, boolean makeConstant) {
    ...
    return Ast.WParameter(source(p), modifiers, transformTypeExpr(p.typeExpr()), text(p.name));
  }
  ...
  private WShortParameter transformShortFormalParameter(ShortFormalParameterContext p) {
    ...
    return Ast.WShortParameter(
        source(p), modifiers, transformOptionalType(p.typeExpr()), text(p.name));
  }
  ...
  private WImport transformImport(WImportContext i) {
    // TODO initlater
    return Ast.WImport(
        source(i), i.isPublic != null, i.isInitLater != null, text(i.importedPackage));
  }
  ...
  static class ClassSlotResult {
    ...
    public ModuleInstanciations moduleInstanciations = Ast.ModuleInstanciations();
