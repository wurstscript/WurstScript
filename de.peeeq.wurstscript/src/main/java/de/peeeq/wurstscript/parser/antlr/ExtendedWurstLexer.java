...
public class ExtendedWurstLexer implements TokenSource {
    ...
    for (; ; ) {
      ...
      if (isWurst) {
        if (isJassOnlyKeyword(token1)) {
          token1 =
              makeToken(
                  WurstParser.ID, token1.getText(), token1.getStartIndex(), token1.getStopIndex());
          assert token1 != null;
      ...
      if (token.getType() == WurstParser.NL) {
        ...
        if (isWurst) {
          ...
          nextTokens.add(
              makeToken(
                  WurstParser.ENDPACKAGE,
                  "endpackage",
                  token.getStartIndex(),
                  token.getStopIndex()));
          nextTokens.add(
              makeToken(WurstParser.NL, "$NL", token.getStartIndex(), token.getStopIndex()));
        }
        ...
        case BEGIN_LINE:
          if (isTab(token)) {
            ...
            if (lastCharWasWrap && numberOfTabs > indentationLevels.peek()) {
              ...
              handleIndent(
                  numberOfTabs, token, token.getStartIndex(), token.getStopIndex(), firstNewline);
              state(State.INIT);
              nextTokens.add(token);
              return firstNewline;
            }
    ...
    if (tabChoice == TabChoice.Unknown) {
      ...
      if (token.getType() == WurstParser.TAB) {
        if (tabChoice == TabChoice.Spaces) {
          tabWarning =
              new CompileError(
                  new WPos("", lineOffsets, token.getStartIndex(), token.getStopIndex()),
                  "Mixing tabs and spaces for indentation.");
        }
      } else if (token.getType() == WurstParser.SPACETAB && tabChoice == TabChoice.Tabs) {
        if (tabWidth(token) > 3) {
  ...
  private boolean isTab(Token token) {
    return token.getType() == WurstParser.TAB || token.getType() == WurstParser.SPACETAB;
  }
    ...
    if (n > indentationLevels.peek()) {
      if (spacesPerIndent < 0) {
        spacesPerIndent = n;
          ...
          && tabWarning == null
          && n != indentationLevels.peek() + spacesPerIndent) {
        String message =
            "Inconsistent indentation: Earlier in this file "
                + spacesPerIndent
                + " spaces were used for indentation and here it is "
                + (n - indentationLevels.peek())
                + " spaces.";
        tabWarning =
            new CompileError(
                new WPos("", lineOffsets, lineOffsets.get(token.getLine()), token.getStopIndex()),
                message);
      }
      if (tabWarning == null && n % 2 == 1) {
        tabWarning =
            new CompileError(
                new WPos("", lineOffsets, lineOffsets.get(token.getLine()), token.getStopIndex()),
                "Use an even number of spaces for indentation.");
      }
      ...
      while (n < indentationLevels.peek()) {
        ...
        nextTokens.add(
            makeToken(
                WurstParser.ENDBLOCK,
                "$end",
                endBlockToken.getStartIndex(),
                endBlockToken.getStartIndex()));
      }
      ...
      if (n != expectedIndentation) {
        ...
        if (token.getType() != WurstParser.END) {
          String msg =
              "Invalid indentation level. Current indentation is "
                  + expectedIndentation
                  + ", but this is indented by "
                  + n
                  + ".";
