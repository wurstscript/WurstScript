...
public class Utils {
  ...
  public static int parseAsciiInt(String yytext) throws NumberFormatException {
    ...
    if (chars != 1 && chars != 4) {
      throw new NumberFormatException(
          "Ascii ints must have 4 or 1 characters but this one has " + chars + " characters.");
    }
    return result;
  }
  ...
  /**
   * sorts a list with partitial ordering topologically. If a > b then a will appear before b in the
   * result list
   *
   ...
   */
  public static <T> List<T> topSort(
      Collection<T> items, java.util.function.Function<T, ? extends Collection<T>> biggerItems)
  ...
  private static <T> void topSortHelper(
      List<T> result,
      Set<T> visitedItems,
  ...
  public static Optional<Element> getAstElementAtPos(
      Element elem, int caretPosition, boolean usesMouse) {
  ...
  public static Optional<Element> getAstElementAtPos(
      Element elem, int line, int column, boolean usesMouse) {
  ...
  public static Optional<Element> getAstElementAtPosIgnoringLists(
      Element elem, int caretPosition, boolean usesMouse) {
  ...
  private static Optional<Element> bestResult(List<Element> betterResults) {
    ...
    for (Element e : betterResults) {
      ...
      int size =
          source.isArtificial() ? Integer.MAX_VALUE : source.getRightPos() - source.getLeftPos();
      if (size < minSize) {
        minSize = size;
        min = Optional.of(e);
      }
  ...
  public static String printElementWithSource(Optional<Element> e) {
    ...
    return printElement(e) + " (" + src.map(WPos::getFile) + ":" + src.map(WPos::getLine) + ")";
  }
  ...
  public static <T extends Comparable<? extends T>, S> TreeMap<T, Set<S>> inverseMapToSet(
      Map<S, T> orig) {
  ...
  public static <T extends Element> Optional<T> getNearestByType(
      Optional<Element> e, Class<T> clazz) {
    while (e.isPresent()) {
  ...
  public static <K, V> ImmutableMap<K, V> mergeMaps(
      ImmutableMap<K, V> a, ImmutableMap<K, V> b, Function2<V, V, V> mergeFunc) {
   ...
   *
   * <p>see http://stackoverflow.com/questions/309424/read-convert-an-inputstream-to-a-string
   */
  ...
  /**
   * Extracts a resource from the jar, stores it in a temp file and returns the abolute path to the
   * tempfile
   */
  ...
  public static void replace(
      de.peeeq.wurstscript.jassIm.Element parent,
      de.peeeq.wurstscript.jassIm.Element oldElement,
      de.peeeq.wurstscript.jassIm.Element newElement) {
    ...
    throw new CompileError(
        parent.attrTrace().attrSource(), "Could not find " + oldElement + " in " + parent);
  }
  ...
  public static ExecResult exec(ProcessBuilder pb, Duration duration, Consumer<String> onInput)
