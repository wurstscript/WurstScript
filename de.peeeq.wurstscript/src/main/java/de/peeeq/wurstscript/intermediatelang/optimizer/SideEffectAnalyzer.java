...
public class SideEffectAnalyzer {
  ...
  /**
   * checks if this expression might have side effects (does not do a deep analysis, all function
   * calls and statements are considered to have side effects)
   */
  public static boolean quickcheckHasSideeffects(ImExpr expr) {
    return expr.match(
        new ImExpr.Matcher<Boolean>() {
          @Override
  ...
  public Set<ImFunction> calledFunctions(Element e) {
    return calledFunctionsStream(e).collect(Collectors.toSet());
  }
  ...
  public Set<ImFunction> calledNatives(Element e) {
    return calledFunctionsStream(e).filter(ImFunction::isNative).collect(Collectors.toSet());
  }

  /**
   * Variables directly or indirectly (via called functions) used in e Does not consider variables
   * used because of natives doing stuff (e.g. ForGroup callback)
   */
  public Set<ImVar> usedVariables(Element e) {
    Stream<ImVar> indirectGlobals =
        calledFunctionsStream(e).flatMap(f -> getUsedGlobals().get(f).stream());
  ...
  /**
   * Checks if two statements might affect each other. When this returns true, it is certain that it
   * does not matter whether stmt1 or stmt2 are called first
   *
   * <p>The only difference between executing stmt1; stmt2 vs stmt2; stmt1 would be if one of the
   * statement crashes and thus the second statement would not be executed. But for optimizations,
   * we assume the program already is correct and thus we can ignore crashes.
   */
