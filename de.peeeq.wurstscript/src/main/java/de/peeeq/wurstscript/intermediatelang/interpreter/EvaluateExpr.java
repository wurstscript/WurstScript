...
public class EvaluateExpr {
  ...
  public static @Nullable ILconst eval(
      ImFunctionCall e, ProgramState globalState, LocalState localState) {
    ImFunction f = e.getFunc();
    ImExprs arguments = e.getArguments();
    return evaluateFunc(globalState, localState, f, arguments, e);
  }

  public static @Nullable ILconst evaluateFunc(
  ...
  @Nullable
  private static ILconst evaluateFunc(
      ProgramState globalState, ImFunction f, Element trace, ILconst[] args) {
  ...
  public static ILconst eval(
      ImOperatorCall e, final ProgramState globalState, final LocalState localState) {
    final ImExprs arguments = e.getArguments();
    WurstOperator op = e.getOp();
    if (arguments.size() == 2 && op.isBinaryOp()) {
      return op.evaluateBinaryOperator(
          arguments.get(0).evaluate(globalState, localState),
          () -> arguments.get(1).evaluate(globalState, localState));
  ...
  public static ILaddress evaluateLvalue(
      ImStatementExpr e, ProgramState globalState, LocalState localState) {
    e.getStatements().runStatements(globalState, localState);
    ...
    if (tupleE instanceof ILconstTuple) {
      ...
      if (e.getTupleIndex() >= t.values().size()) {
        throw new InterpreterException(
            globalState, "Trying to get element " + e.getTupleIndex() + " of tuple value " + t);
      }
    ...
    if (var.isGlobal()) {
      ...
      if (r == null) {
        ...
        if (initExpr != null) {
          ...
          throw new InterpreterException(
              globalState, "Variable " + var.getName() + " is not initialized.");
        }
      ...
      return notNull(
          localState.getVal(var), var.getType(), "Local variable " + var + " is null.", true);
    }
  }
  ...
  private static ILconst notNull(
      @Nullable ILconst val, ImType imType, String msg, boolean failOnErr) {
    if (val == null) {
      if (failOnErr) {
  ...
  public static ILconst eval(ImVarArrayAccess e, ProgramState globalState, LocalState localState) {
    ...
    if (e.getVar().isGlobal()) {
      return notNull(
          globalState.getArrayVal(e.getVar(), indexes),
          e.getVar().getType(),
          "Variable " + e.getVar().getName() + " is null.",
          false);
    } else {
      return notNull(
          localState.getArrayVal(e.getVar(), indexes),
          e.getVar().getType(),
          "Variable " + e.getVar().getName() + " is null.",
          false);
    }
  ...
  public static @Nullable ILconst eval(
      ImMethodCall mc, ProgramState globalState, LocalState localState) {
    ILconstObject receiver =
        globalState.toObject(mc.getReceiver().evaluate(globalState, localState));
  ...
  public static ILconst eval(ImMemberAccess ma, ProgramState globalState, LocalState localState) {
    ILconstObject receiver =
        globalState.toObject(ma.getReceiver().evaluate(globalState, localState));
    if (receiver == null) {
      throw new InterpreterException(ma.getTrace(), "Null pointer dereference");
    }
    List<Integer> indexes =
        ma.getIndexes().stream()
  ...
  public static ILconst eval(ImAlloc imAlloc, ProgramState globalState, LocalState localState) {
    return globalState.allocate(imAlloc.getClazz(), imAlloc.attrTrace());
  }

  public static ILconst eval(ImDealloc imDealloc, ProgramState globalState, LocalState localState) {
  ...
  public static ILconst eval(ImInstanceof e, ProgramState globalState, LocalState localState) {
    ILconstObject obj = globalState.toObject(e.getObj().evaluate(globalState, localState));
    return ILconstBool.instance(
        globalState.isInstanceOf(obj, e.getClazz().getClassDef(), e.attrTrace()));
  }

  public static ILconst eval(ImTypeIdOfClass e, ProgramState globalState, LocalState localState) {
    return new ILconstInt(e.getClazz().getClassDef().attrTypeId());
  }

  public static ILconst eval(ImTypeIdOfObj e, ProgramState globalState, LocalState localState) {
    ILconstObject obj = globalState.toObject(e.getObj().evaluate(globalState, localState));
    return new ILconstInt(globalState.getTypeId(obj, e.attrTrace()));
  }

  public static ILconst eval(ImGetStackTrace e, ProgramState globalState, LocalState localState) {
  ...
  public static ILconst eval(
      ImCompiletimeExpr expr, ProgramState globalState, LocalState localState) {
  ...
  public static AtomicReference<ILconst> compiletimeEvaluationResult(
      ImCompiletimeExpr imCompiletimeExpr) {
    return new AtomicReference<>();
  }

  public static ILaddress evaluateLvalue(
      ImVarAccess va, ProgramState globalState, LocalState localState) {
    ImVar v = va.getVar();
    State state;
  ...
  public static ILaddress evaluateLvalue(
      ImVarArrayAccess va, ProgramState globalState, LocalState localState) {
    ImVar v = va.getVar();
    State state;
    state = v.isGlobal() ? globalState : localState;
    List<Integer> indexes =
  ...
  public static ILaddress evaluateLvalue(
      ImTupleSelection ts, ProgramState globalState, LocalState localState) {
  ...
  public static ILaddress evaluateLvalue(
      ImMemberAccess va, ProgramState globalState, LocalState localState) {
    ImVar v = va.getVar();
    ILconstObject receiver =
        globalState.toObject(va.getReceiver().evaluate(globalState, localState));
    List<Integer> indexes =
        va.getIndexes().stream()
            .map(ie -> ((ILconstInt) ie.evaluate(globalState, localState)).getVal())
            .collect(Collectors.toList());
    return new ILaddress() {
      ...
      @Override
      public ILconst get() {
        return receiver.get(v, indexes).orElseGet(() -> va.attrTyp().defaultValue());
      }
    };
  }

  public static ILaddress evaluateLvalue(
      ImTupleExpr e, ProgramState globalState, LocalState localState) {
    ...
    return new ILaddress() {
      ...
      @Override
      public ILconst get() {
        return new ILconstTuple(addresses.stream().map(ILaddress::get).toArray(ILconst[]::new));
      }
    };
  }
