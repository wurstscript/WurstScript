 ...
 *
 * <p>the input must be a flattened program
 */
public class LocalMerger implements OptimizerPass {
  ...
  private void mergeLocals(Map<ImStmt, Set<ImVar>> livenessInfo, ImFunction func) {
    ...
    PriorityQueue<ImVar> vars =
        new PriorityQueue<>(
            (ImVar a, ImVar b) -> inferenceGraph.get(b).size() - inferenceGraph.get(a).size());
    ...
    while (!vars.isEmpty()) {
      ...
      // check if there is some other variable which is already assigned, has the same type and does
      // not interfere
      nextAssigned:
  ...
  private Map<ImVar, Set<ImVar>> calculateInferenceGraph(Map<ImStmt, Set<ImVar>> livenessInfo) {
    ...
    for (ImStmt s : keys) {
      ...
      for (ImVar v1 : live) {
        ...
        inferenceSet =
            inferenceSet.addAll(live.filter(v2 -> v1.getType().equalsType(v2.getType())));
  ...
  public Map<ImStmt, Set<ImVar>> calculateLiveness(ImFunction func) {
    ...
    while (!todo.isEmpty()) {
      ...
      Set<ImVar> newOut =
          node.getSuccessors().stream().map(in::get).reduce(HashSet.empty(), Set::union);
