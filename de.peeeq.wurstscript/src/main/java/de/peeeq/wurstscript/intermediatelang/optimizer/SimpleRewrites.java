...
public class SimpleRewrites implements OptimizerPass {
  ...
  private void optimizeConsecutiveExitWhen(ImExitwhen lookback, ImExitwhen element) {
    ...
    lookback.setCondition(
        JassIm.ImOperatorCall(
            WurstOperator.OR,
            JassIm.ImExprs(lookback.getCondition().copy(), element.getCondition())));
  ...
  /**
   * Rewrites if statements that only contain an exitwhen statement so that the if's condition is
   * combined with the exitwhen
   *
   * <p>if expr1 exitwhen expr2
   *
   * <p>to:
   *
   * <p>exitwhen expr1 and expr2
   */
  private void optimizeIfExitwhen(ImIf imIf) {
    ...
    imStmt.setCondition(
        (JassIm.ImOperatorCall(
            WurstOperator.AND, JassIm.ImExprs(imIf.getCondition(), imStmt.getCondition()))));
  ...
  private void optimizeOpCall(ImOperatorCall opc) {
    ...
    if (opc.getArguments().size() > 1) {
      ...
      if (left instanceof ImBoolVal && right instanceof ImBoolVal) {
        ...
        if (left instanceof ImStringVal) {
        ...
        } else if (((ImStringVal) right).getValS().equalsIgnoreCase("")
            && opc.getOp() == WurstOperator.PLUS) {
          left.setParent(null);
          opc.replaceBy(left);
          wasViable = true;
        } else {
          ...
          && (left.attrTyp().equalsType(TypesHelper.imInt())
              || left.attrTyp().equalsType(TypesHelper.imReal()))
        ...
        switch (inner.getOp()) {
          ...
          case AND:
            ...
            args.forEach(
                (e) ->
                    imExprs.add(
                        JassIm.ImOperatorCall(WurstOperator.NOT, JassIm.ImExprs(e.copy()))));
  ...
  private boolean optimizeRealReal(
      ImOperatorCall opc, boolean wasViable, ImRealVal left, ImRealVal right) {
  ...
  private boolean optimizeIntInt(
      ImOperatorCall opc, boolean wasViable, ImIntVal left, ImIntVal right) {
    int i1 = left.getValI();
  ...
  private void optimizeIf(ImIf imIf) {
    if (imIf.getThenBlock().isEmpty() && imIf.getElseBlock().isEmpty()) {
      ...
      if (boolVal.getValB()) {
        ...
        && imIf.getThenBlock().size() == 1
        && imIf.getThenBlock().get(0) instanceof ImExitwhen) {
   ...
   * Optimizes
   *
   * <p>set x = expr1 set x = x ⊕ expr2
   *
   * <p>into:
   *
   * <p>set x = expr1 ⊕ expr2
   *
   * <p>like code that is created by the branch merger
   */
