...
public class TempMerger implements OptimizerPass {
  ...
  private void optimizeStatements(ImStmts stmts) {
    ...
    do { // repeat while there are changes found
      ...
      // FIXME this is no longer true, because assignments which are used more than once are not
      // removed
      for (ImStmt s : stmts) {
  ...
  class Knowledge {
    ...
    public void invalidateGlobals() {
      ...
      currentValues
          .entrySet()
          .removeIf(e -> readsGlobal(e.getValue().getRight()) || containsFuncCall(e.getValue()));
    }
