...
public class ILInterpreter implements AbstractInterpreter {
  ...
  public ILInterpreter(
      ImProg prog, WurstGui gui, Optional<File> mapFile, ProgramState globalState) {
    this.prog = prog;
  ...
  public static LocalState runFunc(
      ProgramState globalState, ImFunction f, @Nullable Element caller, ILconst... args) {
    ...
    try {
      ...
      if (f.getParameters().size() != args.length) {
        throw new Error(
            "wrong number of parameters when calling func "
                + f.getName()
                + "("
      ...
      throw new InterpreterException(trace, "You encountered a bug in the interpreter: " + e, e)
          .setStacktrace(msg);
    }
  }
  ...
  private static LocalState runBuiltinFunction(
      ProgramState globalState, ImFunction f, ILconst... args) {
    ...
    globalState.compilationError(
        "function " + f.getName() + " cannot be used from the Wurst interpreter.\n" + errors);
    if (f.getReturnType() instanceof ImVoid) {
      return new LocalState();
    }
    ILconst returnValue =
        ImHelper.defaultValueForComplexType(f.getReturnType())
            .evaluate(globalState, new LocalState());
  ...
  public int getInstanceCount(int val) {
    return (int)
        globalState.getAllObjects().stream()
