...
/**
 * Splits a long function into several smaller functions which are executed using TriggerEvaluate
 */
public class FunctionSplitter {
    ...
    return s.match(
        new ImStmt.Matcher<Integer>() {
          ...
          public Integer case_ImMethodCall(ImMethodCall s) {
            return estimateFuel(s.getArguments()) + estimateFuelMethod(s.getMethod()) + 10;
          }
          ...
          public Integer case_ImVarargLoop(ImVarargLoop s) {
            throw new CompileError(
                s,
                "Cannot estimate size of function " + func.getBody() + " as it contains looops.");
          }
          ...
          public Integer case_ImMemberAccess(ImMemberAccess s) {
            return 1 + estimateFuel(s.getReceiver()) + estimateFuel(s.getIndexes());
          }
          ...
          public Integer case_ImIf(ImIf s) {
            return 1
                + estimateFuel(s.getCondition())
                + Math.max(estimateFuel(s.getThenBlock()), estimateFuel(s.getElseBlock()));
          }
  ...
  private int estimateFuelMethod(ImMethod method) {
    return Math.max(
        ...
        method.getSubMethods().stream().mapToInt(m -> estimateFuelMethod(method)).sum());
  }
