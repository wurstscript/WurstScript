...
public class ImportFile {
  ...
  public static void extractImportsFromMap(File mapFile, RunArgs runArgs) {
    ...
    if (!mapFile.exists() || !mapFile.isFile() || mapFile.getName().endsWith("w3m")) {
      JOptionPane.showMessageDialog(
          null,
          "Map " + mapFile.getAbsolutePath() + " does not exist or is of wrong format (w3x only)");
      return;
    }
  ...
  private static void extractImportsFrom(File importDirectory, File tempMap, RunArgs runArgs)
      throws Exception {
    try (MpqEditor editor = MpqEditorFactory.getEditor(Optional.of(tempMap))) {
      ...
      if (failed.isEmpty()) {
        JOptionPane.showMessageDialog(
            null, "All imports were extracted to " + importDirectory.getAbsolutePath());
      } else {
  ...
  private static LinkedList<String> extractImportedFiles(MpqEditor mpq, File directory) {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          null, "No vaild war3map.imp was found, or there are no imports");
      return failed;
    }
    try (LittleEndianDataInputStream reader =
        new LittleEndianDataInputStream(new ByteArrayInputStream(temp))) {
  ...
  private static void extractImportedFile(
      MpqEditor mpq, File directory, LinkedList<String> failed, LittleEndianDataInputStream reader)
      throws Exception {
  ...
  private static void insertImportedFiles(MpqEditor mpq, File directory) throws Exception {
    ...
    dataOut.write(
        ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putInt(FILE_VERSION).array());
    dataOut.write(
        ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putInt(files.size()).array());
    for (File f : files) {
      Path p = f.toPath();
  ...
  public static void importFilesFromImportDirectory(File projectFolder, MpqEditor ed) {
    ...
    if (importDirectory.exists() && importDirectory.isDirectory()) {
      ...
      try {
        ...
        WLogger.severe(e);
        JOptionPane.showMessageDialog(
            null, "Couldn't import resources from " + importDirectory + ": " + e.getMessage());
      }
    }
  }
