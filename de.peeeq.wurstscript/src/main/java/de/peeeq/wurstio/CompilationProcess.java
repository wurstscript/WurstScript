...
public class CompilationProcess {
  ...
  @Nullable
  CharSequence doCompilation(@Nullable MpqEditor mpqEditor) throws IOException {
    return doCompilation(mpqEditor, null);
  }

  @Nullable
  CharSequence doCompilation(@Nullable MpqEditor mpqEditor, @Nullable File projectFolder)
      throws IOException {
    WurstCompilerJassImpl compiler =
        new WurstCompilerJassImpl(timeTaker, projectFolder, gui, mpqEditor, runArgs);
    ...
    WurstModel model = timeTaker.measure("parse files", compiler::parseFiles);
    ...
    timeTaker.measure("Typecheck program", () -> compiler.checkProg(model));

    if (gui.getErrorCount() > 0) {
      return null;
    }

    timeTaker.measure("Translate program to Im", () -> compiler.translateProgToIm(model));
    ...
    timeTaker.measure("Run compiletime functions", compiler::runCompiletime);

    JassProg jassProg =
        timeTaker.measure("Transform program to Jass", compiler::transformProgToJass);
    ...
    CharSequence mapScript = timeTaker.measure("Print Jass", (Supplier<String>) printer::printProg);

    // output to file
    File outputMapscript = timeTaker.measure("Print Jass", () -> writeMapscript(mapScript));

    if (!runArgs.isDisablePjass()) {
      boolean pjassError = timeTaker.measure("Run PJass", () -> runPjass(outputMapscript));
  ...
  private boolean runPjass(File outputMapscript) {
    ...
    if (commonJ.exists() && blizzJ.exists()) {
      pJassResult =
          Pjass.runPjass(outputMapscript, commonJ.getAbsolutePath(), blizzJ.getAbsolutePath());
    } else {
  ...
  private void runTests(ImTranslator translator, WurstCompilerJassImpl compiler, int testTimeout) {
    ...
    RunTests runTests =
        new RunTests(Optional.empty(), 0, 0, Optional.empty(), testTimeout) {
          @Override
