...
public class CompiletimeFunctionRunner {
  ...
  private final Map<ImFunction, Pair<de.peeeq.wurstscript.jassIm.Element, String>> failTests =
  ...
  private void collectCompiletimeExpressions(
      List<Either<ImCompiletimeExpr, ImFunction>> toExecute) {
    imProg.accept(
  ...
  private GetAForB<ILconstObject, ImVar> globalForObject =
      new GetAForB<ILconstObject, ImVar>() {
        @Override
        public ImVar initFor(ILconstObject obj) {

          ImVar res =
              JassIm.ImVar(obj.getTrace(), obj.getType(), obj.getType() + "_compiletime", false);
          ...
          delayedActions.add(
              () -> {
                for (Map.Entry<ImVar, Map<List<Integer>, ILconst>> entry :
                    obj.getAttributes().rowMap().entrySet()) {
                  ...
                  for (Map.Entry<List<Integer>, ILconst> entry2 : value1.entrySet()) {
                    ...
                    addCompiletimeStateInit(
                        JassIm.ImSet(
                            trace,
                            JassIm.ImMemberAccess(
                                trace,
                                JassIm.ImVarAccess(res),
                                JassIm.ImTypeArguments(),
                                var,
                                indexesT),
                            constantToExpr(trace, attrValue)));
                  }
                }
              });
  ...
  private GetAForB<IlConstHandle, ImVar> globalForHandle =
      new GetAForB<IlConstHandle, ImVar>() {
        @Override
        public ImVar initFor(IlConstHandle a) {
          ...
          if (obj instanceof ArrayListMultimap) {
            ...
            ArrayListMultimap<HashtableProvider.KeyPair, Object> map =
                (ArrayListMultimap<HashtableProvider.KeyPair, Object>) obj;
            ...
            throw new RuntimeException(
                "Handle value "
                    ...
                    + " ("
                    + obj.getClass()
                    + ") can not be persistet at compiletime");
          }
        }
      };
  ...
  private ImExpr constantToExpr(Element trace, ILconst value) {
    ...
    throw new InterpreterException(
        trace, "Compiletime expression returned unsupported value " + value);
  }
    ...
    if (res == null) {
      ...
      ImFunctionCall call =
          JassIm.ImFunctionCall(
              trace, res, JassIm.ImTypeArguments(), JassIm.ImExprs(), true, CallType.NORMAL);
  ...
  private void addCompiletimeStateInitAlloc(Element trace, ImVar v, ImExpr init) {
    ...
    getCompiletimeStateInitFunction()
        .getBody()
        .add(0, JassIm.ImSet(trace, JassIm.ImVarAccess(v), init.copy()));
  }
  ...
  /**
   * Stores a hashtable value in a compiletime expression by generating the respective native calls
   */
  private ImExpr constantToExprHashtable(
      Element trace,
      ImVar htVar,
      IlConstHandle handle,
      ArrayListMultimap<HashtableProvider.KeyPair, Object> map) {
          ...
          for (Map.Entry<HashtableProvider.KeyPair, Object> entry : map.entries()) {
            ...
            if (v instanceof ILconstInt) {
              ...
              throw new CompileError(
                  errorPos,
                  "Unsupported value stored in HashMap: "
                      + v
                      + " // "
    ...
    return JassIm.ImFunctionCall(
        trace, initHashtable, JassIm.ImTypeArguments(), JassIm.ImExprs(), false, CallType.NORMAL);
  }

  @NotNull
  private ImFunction findNative(String funcName, WPos trace) {
    return imProg.getFunctions().stream()
