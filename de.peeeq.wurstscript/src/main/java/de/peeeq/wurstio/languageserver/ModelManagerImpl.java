...
public class ModelManagerImpl implements ModelManager {
  ...
  private List<CompilationUnit> getJassdocCUs(Path jassdoc, WurstGui gui) {
    ...
    WurstCompilerJassImpl comp =
        new WurstCompilerJassImpl(projectPath, gui, null, RunArgs.defaults());
  ...
  /**
   * clear the attributes for all compilation units that import something from 'toCheck' and for
   * 'toCheck'
   *
  ...
  private boolean imports(
      WPackage p, Set<String> packageNames, boolean onlyPublic, HashSet<WPackage> visited) {
  ...
  private void doTypeCheck(WurstGui gui) {
    ...
    @Nullable WurstModel model2 = model;
    if (model2 == null) {
      return;
    }
  ...
  private void reportErrorsForProject(String extra, WurstGui gui) {
    ...
    for (WFile file : parseErrors.keySet()) {
      List<CompileError> errors =
          new ArrayList<>(parseErrors.getOrDefault(file, Collections.emptyList()));
      ...
      try {
        ...
        if (!sourceFile.exists()) {
          java.nio.file.Files.copy(
              source, sourceFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
        }
      } finally {
        source.close();
      }
  ...
  @Override
  public CompilationUnit replaceCompilationUnitContent(
      WFile filename, String contents, boolean reportErrors) {
  ...
  private CompilationUnit replaceCompilationUnit(
      WFile filename, String contents, boolean reportErrors) {
  ...
  private void doTypeCheckPartial(
      WurstGui gui, List<WFile> toCheckFilenames, Set<String> oldPackages) {
  ...
  private Set<CompilationUnit> addPackageDependencies(
      List<CompilationUnit> toCheck, Set<String> oldPackages, WurstModel model) {

    Set<CompilationUnit> result =
        new TreeSet<>(Comparator.comparing(cu -> cu.getCuInfo().getFile()));
    ...
    Stream<String> providedPackages =
        result.stream().flatMap(cu -> cu.getPackages().stream()).map(WPackage::getName);

    Set<String> affectedPackages =
        Stream.concat(providedPackages, oldPackages.stream()).collect(Collectors.toSet());
  ...
  private void addImportingPackages(
      Collection<String> providedPackages, WurstModel model2, Set<CompilationUnit> result) {
    nextCu:
  ...
  private boolean isInWurstFolder(WFile file) {
    return Stream.concat(Stream.of(projectPath), dependencies.stream())
        .anyMatch(p -> FileUtils.isInDirectoryTrans(file, WFile.create(new File(p, "wurst"))));
  }
