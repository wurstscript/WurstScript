  ...
  public Object execute(ModelManager modelManager) throws IOException {
    if (modelManager.hasErrors()) {
      throw new RequestFailedException(
          MessageType.Error, "Fix errors in your code before building a release.");
    }
    ...
    try {
      ...
      Optional<File> targetMap =
          Optional.of(
              new File(
                  buildDir,
                  fileName.isEmpty()
                      ? projectConfig.getProjectName() + ".w3x"
                      : fileName + ".w3x"));
      ...
      ProjectConfigBuilder.apply(
          projectConfig, targetMap.get(), compiledScript, buildDir, runArgs, w3data);
      ...
      WLogger.info(e);
      throw new RequestFailedException(
          MessageType.Error, "A compilation error occurred when building the map:\n" + e);
    } catch (Exception e) {
      WLogger.warning("Exception occurred", e);
      throw new RequestFailedException(
          MessageType.Error, "An exception was thrown when building the map:\n" + e);
    } finally {
