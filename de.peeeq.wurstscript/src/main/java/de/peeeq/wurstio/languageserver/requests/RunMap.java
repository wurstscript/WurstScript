...
public class RunMap extends MapRequest {
  ...
  public Object execute(ModelManager modelManager) throws IOException {
    ...
    if (modelManager.hasErrors()) {
      throw new RequestFailedException(
          MessageType.Error, "Fix errors in your code before running.");
    }

    WurstProjectConfigData projectConfig =
        WurstProjectConfig.INSTANCE.loadProject(
            workspaceRoot.getFile().toPath().resolve(FILE_NAME));
    ...
    WLogger.info(
        "received runMap command: map=" + map + ", wc3dir=" + wc3Path + ", args=" + compileArgs);
    WurstGui gui = new WurstGuiImpl(getWorkspaceAbsolute());
    try {
      if (map.isPresent() && !map.get().exists()) {
        throw new RequestFailedException(
            MessageType.Error, map.get().getAbsolutePath() + " does not exist.");
      }
      ...
      if (testMap.isPresent()) {
        ...
        ProjectConfigBuilder.apply(
            projectConfig, testMap.get(), compiledScript, buildDir, runArgs, w3data);
      ...
      WLogger.info(e);
      throw new RequestFailedException(
          MessageType.Error, "A compilation error occurred when running the map:\n" + e);
    } catch (Exception e) {
      WLogger.warning("Exception occurred", e);
      throw new RequestFailedException(
          MessageType.Error, "An exception was thrown when running the map:\n" + e);
    } finally {
  ...
  private void callJhcrUpdate(File mapScript) throws IOException, InterruptedException {
    ...
    ProcessBuilder pb =
        new ProcessBuilder(
            ...
            "--asm",
            "--preload-path",
            customMapDataPath.toAbsolutePath().toString());
    pb.directory(mapScriptFolder);
    Utils.ExecResult $ = Utils.exec(pb, Duration.ofSeconds(30), System.err::println);
  }

  /**
   * Tries to find the path where the wc3 CustomMapData is stored. For example this could be in:
   ...
   *
   * <p>This directory depends on warcraft version and whether we are on windows or wine is used.
   */
  private File copyToWarcraftMapDir(File testMap) throws IOException {
    ...
    for (String arg : compileArgs) {
      if (arg.startsWith("-runmapTarget")) {
        ...
        if (customTarget.exists() && customTarget.isDirectory()) {
          ...
          WLogger.severe(
              "Directory specified via -runmapTarget does not exists or is not a directory");
        }
      }
    }
    ...
    Optional<File> testFolder =
        documentPath.map(path -> new File(path, "Maps" + File.separator + "Test"));
        ...
        try {
          ...
          int result =
              JOptionPane.showOptionDialog(
                  jf,
                  "Can't write to target map file, it's probably in use.",
                  "Run Map",
  ...
  private Optional<String> findMapDocumentPath(String testMapName, File myDocumentsFolder) {
    ...
    if (w3data.getWc3PatchVersion().get().compareTo(new GameVersion("1.27.9")) <= 0) {
      ...
      Optional<File> oldFile =
          wc3Path.map(
              w3p ->
                  new File(w3p, "Maps" + File.separator + "Test" + File.separator + testMapName));
