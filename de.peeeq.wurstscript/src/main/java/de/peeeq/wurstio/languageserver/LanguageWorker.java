          ...
          try {
            ...
            languageClient.showMessage(
                new MessageParams(
                    ...
                    "Request '"
                        + work
                        + "' could not be processed (see log for details): "
                        + e.toString()));
            WLogger.severe(e);
            System.err.println(
                "Error in request '" + work + "' (see log for details): " + e.getMessage());
          }
        }
      }
          ...
          (res, err) -> {
            if (err != null) {
              ...
              if (!(request instanceof HoverInfo)) {
                languageClient.showMessage(
                    new MessageParams(MessageType.Error, "Request returned null: " + request));
              }
              resFut.completeExceptionally(
                  new RuntimeException("Request returned null: " + request));
            } else {
              resFut.complete(res);
            }
          });
