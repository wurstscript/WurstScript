...
public class GetCompletions extends UserRequest<CompletionList> {
  ...
  private Comparator<CompletionItem> completionItemComparator() {
    return Comparator.comparing(CompletionItem::getSortText)
        .reversed()
  ...
  private void calculateCompletions(List<CompletionItem> completions) {
    ...
    if (elem instanceof ExprMember) {
      ...
      while (scope != null) {
        ...
        completionsAddVisibleNames(
            alreadyEntered, completions, visibleNames, leftType, isMemberAccess, elem);
   ...
   *
   * <p>checks if the cursor is currently in the parentheses, given the position of the dot
   */
  ...
  private void addDefaultCompletions(
      List<CompletionItem> completions, Element elem, boolean isMemberAccess) {
    WurstType leftType;
  ...
  private void getCompletionsForExistingConstructorCall(
      List<CompletionItem> completions, ExprNewObject c) {
  ...
  private void getCompletionsForExistingMemberCall(
      List<CompletionItem> completions, ExprMemberMethod c) {
  ...
  */
  ...
  /**
   * checks if we are currently entering a real number (autocomplete might have triggered because of
   * the dot)
   */
  ...
  private void completionsAddVisibleNames(
      String alreadyEntered,
      List<CompletionItem> completions,
      Multimap<String, DefLink> visibleNames,
      @Nullable WurstType leftType,
    ...
    for (Entry<String, DefLink> e : entries) {
      ...
      if (defLink.getVisibility() == Visibility.PRIVATE_OTHER
          || defLink.getVisibility() == Visibility.PROTECTED_OTHER) {
        continue;
      }
  ...
  private void addParamSnippet(
      String replacementString,
      List<String> paramNames,
      CompletionItem completion,
      StringBuilder lambdaReplacement) {
  ...
  private CompletionItem makeConstructorCompletion(ClassDef c, ConstructorDef constr) {
    ...
    List<String> parameterNames =
        constr.getParameters().stream().map(WParameter::getName).collect(Collectors.toList());
    addParamSnippet(replacementString, parameterNames, completion, null);

    return completion;
  }

  private void completionsAddVisibleExtensionFunctions(
      List<CompletionItem> completions,
      Multimap<String, DefLink> visibleNames,
      WurstType leftType) {
