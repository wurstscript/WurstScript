...
public class GetUsages extends UserRequest<List<GetUsages.UsagesData>> {
  ...
  public GetUsages(
      TextDocumentPositionParams position, BufferManager bufferManager, boolean global) {
  ...
  public List<UsagesData> execute(ModelManager modelManager) {
    ...
    if (nameDef.isPresent()) {
      ...
      if (global || nameDef.get().getSource().getFile().equals(wFile.toString())) {
        ...
        usages.add(
            new UsagesData(
                Convert.posToLocation(nameDef.get().attrErrorPos()), DocumentHighlightKind.Write));
      }
      ...
      while (!todo.isEmpty()) {
        ...
        if (e_def == nameDef.get()) {
          UsagesData usagesData =
              new UsagesData(Convert.posToLocation(e.attrErrorPos()), DocumentHighlightKind.Read);
