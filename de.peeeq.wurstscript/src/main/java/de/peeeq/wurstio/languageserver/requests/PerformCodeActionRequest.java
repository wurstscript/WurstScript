...
public class PerformCodeActionRequest extends UserRequest<Object> {
  ...
  public Object execute(ModelManager modelManager) {
    ...
    switch (action.get("type").getAsString()) {
      case IMPORT_PACKAGE:
        return addImport(
            modelManager,
            action.get("uriString").getAsString(),
            action.get("import").getAsString());
      case INSERT_CODE:
        return insertCodeAction(
            modelManager,
            action.get("uriString").getAsString(),
            action.get("line").getAsInt(),
            action.get("insertedFunction").getAsString());
    }
    throw new RuntimeException("Unhandled action: " + action);
  }

  private Object insertCodeAction(
      ModelManager modelManager, String fileUri, int line, String insertedFunction) {
  ...
  private Object applyTextEdits(WFile file, List<TextEdit> textEdits) {
    ...
    VersionedTextDocumentIdentifier textDocument =
        new VersionedTextDocumentIdentifier(file.getUriString(), version);
    ...
    List<Either<TextDocumentEdit, ResourceOperation>> documentChanges =
        Collections.singletonList(Either.forLeft(change));
