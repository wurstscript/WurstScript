...
public class RunTests extends UserRequest<Object> {
  ...
  public Object execute(ModelManager modelManager) {
    if (modelManager.hasErrors()) {
      throw new RequestFailedException(
          MessageType.Error, "Fix errors in your code before running tests.");
    }
  ...
  public TestResult runTests(
      ...
      ImProg imProg,
      Optional<FuncDef> funcToTest,
      Optional<CompilationUnit> cu) {
    WurstGui gui = new TestGui();

    CompiletimeFunctionRunner cfr =
        new CompiletimeFunctionRunner(
            translator, imProg, Optional.empty(), null, gui, CompiletimeFunctions);
    ...
    for (ImFunction f : imProg.getFunctions()) {
      if (f.hasFlag(FunctionFlagEnum.IS_TEST)) {
        ...
        String message =
            "Running <"
                + f.attrTrace().attrNearestPackage().tryGetNameDef().getName()
                + ":"
                + f.attrTrace().attrErrorPos().getLine()
                + " - "
                + f.getName()
                + ">..";
        println(message);
        ...
        try {
          ...
          println(
              "\tFAILED - TIMEOUT (This test did not complete in "
                  + timeoutSeconds
                  + " seconds, it might contain an endless loop)");
    ...
    println(
        "Tests succeeded: " + successTests.size() + "/" + (successTests.size() + failTests.size()));
    if (failTests.size() == 0) {
  ...
  private Optional<FuncDef> getFunctionToTest(Optional<CompilationUnit> maybeCu) {
    if (testName.isPresent()) {
      ...
      Optional<FuncDef> testFunc =
          maybeCu.flatMap(
              cu ->
                  cu.getPackages().stream()
