...
public class ProgramStateIO extends ProgramState {
  ...
  public ProgramStateIO(
      Optional<File> mapFile,
      @Nullable MpqEditor mpqEditor,
      WurstGui gui,
      ImProg prog,
  ...
  private void deleteWurstObjects(ObjectFile unitStore) {
    Iterator<Map.Entry<Integer, ObjectDefinition>> it =
        unitStore.getModifiedTable().getObjectDefinitions().entrySet().iterator();
    while (it.hasNext()) {
  ...
  private void writeW3oFile() {
    Optional<File> objFile =
        getObjectEditingOutputFolder().map(fo -> new File(fo, "wurstCreatedObjects.w3o"));
  ...
  private void writebackObjectFile(ObjectFile dataStore, ObjectFileType fileType, boolean inject)
      throws Error {
    try {
