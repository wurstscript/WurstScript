...
public class WurstCompilerJassImpl implements WurstCompiler {
  ...
  public WurstCompilerJassImpl(
      @Nullable File projectFolder, WurstGui gui, @Nullable MpqEditor mapFileMpq, RunArgs runArgs) {
    this(new TimeTaker.Default(), projectFolder, gui, mapFileMpq, runArgs);
  }

  public WurstCompilerJassImpl(
      TimeTaker timeTaker,
      @Nullable File projectFolder,
      WurstGui gui,
      @Nullable MpqEditor mapFileMpq,
      RunArgs runArgs) {
  ...
  public void runCompiletime() {
    if (runArgs.runCompiletimeFunctions()) {
      ...
      CompiletimeFunctionRunner ctr =
          new CompiletimeFunctionRunner(
              imTranslator,
              getImProg(),
              getMapFile(),
              getMapfileMpqEditor(),
    ...
    if (gui.getErrorCount() > 0) {
      CompileError compileError = gui.getErrorList().get(0);
      throw new RequestFailedException(
          MessageType.Error,
          "Could not compile project (error in running compiletime functions/expressions): ",
          compileError);
    }
  ...
  public void loadWurstFilesInDir(File dir) {
    for (File f : dir.listFiles()) {
      if (f.isDirectory()) {
        ...
        addDependencyFile(f);
      } else if ((!mapFile.isPresent() || runArgs.isNoExtractMapScript())
          && f.getName().equals("war3map.j")) {
        loadFile(f);
      }
    }
  }
  ...
  private void addDependencyFolder(File f, String folderName) {
    ...
    if (!folder.exists()) {
      gui.sendError(
          new CompileError(
              new WPos(f.getAbsolutePath(), new LineOffsets(), 0, 1),
      ...
      if (file.getName().endsWith(".w3x") || file.getName().endsWith(".w3m")) {
        ...
        if (projectFolder != null && !file.getParent().equals(projectFolder.getAbsolutePath())) {
          throw new RuntimeException(
              "Cannot set projectFolder to "
                  + file
                  + " because it is already set to non parent "
                  + projectFolder);
        }
        projectFolder = file;
      }
  ...
  public void addImportedLibs(
      List<CompilationUnit> compilationUnits, Function<File, CompilationUnit> addCompilationUnit) {
  ...
  private void resolveImport(
      Function<File, CompilationUnit> addCompilationUnit, Set<String> packages, WImport imp)
      throws CompileError {
  ...
  private CompilationUnit loadLibPackage(
      Function<File, CompilationUnit> addCompilationUnit, String imp) {
    File file = getLibs().get(imp);
    if (file == null) {
      gui.sendError(
          new CompileError(
              new WPos("", null, 0, 0),
              "Could not find lib-package "
                  + imp
                  + ". Are you missing your wurst.dependencies file?"));
      return Ast.CompilationUnit(
          new CompilationUnitInfo(errorHandler), Ast.JassToplevelDeclarations(), Ast.WPackages());
    } else {
  ...
  public JassProg transformProgToJass() {
    ...
    new MultiArrayEliminator(
            imProg2, imTranslator2, runArgs.isIncludeStacktraces() && !runArgs.isNoDebugMessages())
        .run();
    ...
    ImToJassTranslator translator =
        new ImToJassTranslator(
            getImProg(),
            getImTranslator().getCalledFunctions(),
            getImTranslator().getMainFunc(),
            getImTranslator().getConfFunc());
  ...
  private void addJassHotCodeReloadCode() {
    ...
    ImFunction jhcr_reload =
        JassIm.ImFunction(
            trace,
            "jhcr_reload_on_escape",
            JassIm.ImTypeVars(),
            JassIm.ImVars(),
            JassIm.ImVoid(),
            JassIm.ImVars(),
    ...
    stmts.add(
        JassIm.ImSet(
            trace, JassIm.ImVarAccess(trig), callExtern(trace, CallType.NORMAL, "CreateTrigger")));
    stmts.add(
        callExtern(
            trace,
            CallType.NORMAL,
            "TriggerRegisterPlayerEventEndCinematic",
            JassIm.ImVarAccess(trig),
  ...
  @NotNull
  private ImFunctionCall callExtern(
      Element trace, CallType callType, String functionName, ImExpr... arguments) {
    ImFunction jhcrinit =
        JassIm.ImFunction(
            trace,
            functionName,
    ...
    return JassIm.ImFunctionCall(
        trace, jhcrinit, JassIm.ImTypeArguments(), JassIm.ImExprs(arguments), true, callType);
  }
  ...
  private void beginPhase(int phase, String description) {
    errorHandler.setProgress(
        "Translating wurst. Phase " + phase + ": " + description, 0.6 + 0.01 * phase);
  ...
  private CompilationUnit processMap(File file) {
    ...
    try {
      ...
      File tempFile =
          File.createTempFile(
              "war3map", ".j", TempDir.get()); // TODO work directly with bytes without temp file
      ...
      if (!wurstFolder.isDirectory()) {
        throw new AbortCompilationException(
            "Could not create Wurst folder at " + wurstFolder + ".");
      }
  ...
  private CompilationUnit parseFile(File file) {
    ...
    try (Reader reader = FileReading.getFileReader(file)) {
      ...
      gui.sendError(
          new CompileError(new WPos(source, LineOffsets.dummy, 0, 0), "Could not read file."));
      return emptyCompilationUnit();
    }
  }
