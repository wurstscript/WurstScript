...
public class ErrorReportingIO extends ErrorReporting {
  ...
  public void handleSevere(final Throwable t, final String sourcecode) {
    ...
    String title = "Sor!";
    String message =
        "You have encountered a bug in the Wurst Compiler.\n"
            + "Your version is: "
            + AboutDialog.version
            + "\n"
    ...
    Object[] options = {"Nothing", "Send automatic error report", "Create manual bug report"};
    ...
    if (n == 1) {
      ...
      threads[1] =
          new Thread(() -> results[1] = sendErrorReport(t, "\n\nLog: \n\n" + WLogger.getLog()));
  ...
  public boolean sendErrorReport(Throwable t, String sourcecode) {
    ...
    try {
      ...
      data +=
          "&"
              + URLEncoder.encode("stacktrace", "UTF-8")
              + "="
              + URLEncoder.encode(Utils.printExceptionWithStackTrace(t), "UTF-8");
      data +=
          "&"
      ...
      data +=
          "&" + URLEncoder.encode("source", "UTF-8") + "=" + URLEncoder.encode(sourcecode, "UTF-8");
      ...
      connection.setRequestProperty(
          "Content-Length", "" + Integer.toString(data.getBytes().length));
  ...
  private String showMultilineMessageDialog() {
    ...
    JComponent inputs[] = {
      new JLabel(
          "Please add some contact information here in case we have further questions regarding this problem."),
    ...
    int r =
        JOptionPane.showConfirmDialog(
            null, inputs, "My custom dialog", JOptionPane.OK_CANCEL_OPTION);
