...
public class Main {
      ...
      try {
        ...
        if (runArgs.isBuild() && runArgs.getInputmap() != null && workspaceroot != null) {
          ...
          if (java.nio.file.Files.exists(inputMap) && projectConfig != null) {
            ...
            target =
                Optional.of(
                    buildDir.resolve(projectConfig.getBuildMapData().getFileName() + ".w3x"));
        ...
        if (mapFilePath != null && workspaceroot != null) {
          try (MpqEditor mpqEditor =
              MpqEditorFactory.getEditor(Optional.of(new File(mapFilePath)))) {
        ...
        if (compiledScript != null) {
          ...
          if (projectConfig != null && target.isPresent()) {
            ProjectConfigBuilder.apply(
                ...
                buildDir.toFile(),
                runArgs,
                new W3InstallationData());

            WLogger.info("map build success");
            System.out.println(
                "Build succeeded. Output file: <" + target.get().toAbsolutePath() + ">");
          }
        }
  ...
  private static void logStartup(String[] args) {
    ...
    try {
      WLogger.info(
          "### compiler path1: " + Main.class.getProtectionDomain().getCodeSource().getLocation());
      WLogger.info(
          "### compiler path2: " + ClassLoader.getSystemClassLoader().getResource(".").getPath());
