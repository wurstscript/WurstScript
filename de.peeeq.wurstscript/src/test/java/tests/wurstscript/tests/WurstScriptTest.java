...
public class WurstScriptTest {
  ...
  class TestConfig {
      ...
      try {
        ...
        if (expectedError != null) {
          if (res.getGui().getErrorCount() == 0) {
            ...
            if (errors.stream()
                .noneMatch(
                    e -> e.getMessage().toLowerCase().contains(expectedError.toLowerCase()))) {
    ...
    private CompilationResult testScript() {
      ...
      if (stopOnFirstError
          && name.toLowerCase().contains("warning")
          && !gui.getWarningList().isEmpty()) {
      ...
      testWithoutInliningAndOptimization(
          name,
          executeProg,
          executeTests,
          gui,
      ...
      testWithInliningAndOptimizationsAndStacktraces(
          name,
          executeProg,
          executeTests,
          gui,
    ...
    public void file(File file) throws IOException {
      try {
        ...
        throw new FileNotFoundException(
            "Failed to open file " + file.getAbsolutePath() + " - can read? " + file.canRead());
      }
      run();
    }
  ...
  public void testAssertErrorsLinesWithStdLib(
      boolean executeProg, String errorMessage, String... input) {
  ...
  void testAssertErrorFileWithStdLib(File file, String errorMessage, boolean executeProg)
      throws IOException {
  ...
  private void testWithInliningAndOptimizations(
      String name,
      boolean executeProg,
      ...
      WurstModel model,
      boolean executeProgOnlyAfterTransforms,
      RunArgs runArgs)
      throws Error {
  ...
  private void testWithInliningAndOptimizationsAndStacktraces(
      String name,
      boolean executeProg,
      boolean executeTests,
      WurstGui gui,
  ...
  private void testWithLocalOptimizations(
      String name,
      boolean executeProg,
      boolean executeTests,
      WurstGui gui,
  ...
  private void testWithoutInliningAndOptimization(
      String name,
      boolean executeProg,
      boolean executeTests,
      WurstGui gui,
    ...
    translateAndTest(
        name, executeProg, executeTests, gui, compiler, model, executeProgOnlyAfterTransforms);
  }

  private void translateAndTestLua(
      String name,
      boolean executeProg,
      WurstGui gui,
      WurstModel model,
    ...
    try {
      ...
      if (executeProg) {
        String line;
        String[] args = {"lua", "-l", luaFile.getPath().replace(".lua", ""), "-e", "main()"};
  ...
  private void translateAndTest(
    ...
    if (!executeProgOnlyAfterTransforms) {
      // we want to test that the interpreter works correctly before transforming the program in the
      // translation step
      if (executeTests) {
  ...
  private void executeJassProg(JassProg prog) throws TestFailException {
    try {
  ...
  private void executeTests(WurstGui gui, ImTranslator translator, ImProg imProg) {
    ...
    RunTests.TestResult res =
        runTests.runTests(translator, imProg, Optional.empty(), Optional.empty());
  ...
  private File writeJassProg(String name, WurstGui gui, JassProg prog) throws Error {
    ...
    try {
      ...
      throw new Error(
          "IOException, could not write jass file " + outputFile + "\n" + gui.getErrors());
    }
    return outputFile;
  }
