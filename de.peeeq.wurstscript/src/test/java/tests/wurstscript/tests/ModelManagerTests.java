...
public class ModelManagerTests {
  ...
  public void test() throws IOException {
    ...
    String packageA_v1 =
        string("package A", "import B", "import C", "public function a()", "	b()", "	c()");

    String packageB_v1 = string("package B", "import C", "public function b_old()", "	c()");

    String packageC_v1 = string("package C", "public function c_old()");
    ...
    assertThat(
        results.get(fileA),
        new IsNot<>(containsString("Reference to function b could not be resolved")));
    assertThat(results.get(fileA), containsString("Reference to function c could not be resolved"));
    assertThat(results.get(fileB), containsString("Reference to function c could not be resolved"));
  ...
  public void movingFiles() throws IOException { // #712
    ...
    String packageA = string("package A", "import Test", "init", "    foo()");

    String packageTest = string("package Test", "public function foo()");
  ...
  public void renamePackage() throws IOException {
    ...
    String packageTest2 = string("package Testttt", "public function foo()");
    ...
    assertThat(
        errors.get(fileA), CoreMatchers.containsString("The import 'Test' could not be resolved."));
  }
  ...
  private Map<WFile, String> keepErrorsInMap(ModelManagerImpl manager) {
    ...
    manager.onCompilationResult(
        (PublishDiagnosticsParams res) -> {
          String errors =
              res.getDiagnostics().stream()
                  .map(e -> e.toString())
