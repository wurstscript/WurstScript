...
public class ParserTests extends WurstScriptTest {
  ...
  public void parenthesis1() {
    testAssertErrorsLines(false, "no", "package test", "	init", "		print(\"hello\" ", "endpackage");
  }

  @Test
  public void parenthesis2() {
    testAssertErrorsLines(
        false, "no", "package test", "	init", "		print(\"hello\")) ", "endpackage");
  }
  ...
  public void dotTo() {
    testAssertErrorsLines(
        false, "extraneous input 'to'", "package Test", "init", "	let x = 1", "	string s = 1.to");
  }

  @Test
  public void indentWithSpaces() {
    testAssertOkLines(
        false, "package Test", "init", "    var x = 1", "    if x > 10", "		x -= 1", "	 x += 1");
  }
    ...
    CompilationResult res =
        test()
            ...
            .lines(
                ...
                "            x = x * 2");
    Assert.assertTrue(
        res.getGui().getWarningList().stream()
            .anyMatch(
                w ->
                    w.getMessage()
                        .contains(
                            "Inconsistent indentation: Earlier in this file 2 spaces were used for indentation and here it is 4 spaces.")));
  }
    ...
    CompilationResult res =
        test()
            ...
            .lines(
                ...
                "            x = x * 2",
                "        end)");
    Assert.assertEquals(Collections.emptyList(), res.getGui().getWarningList());
  }
