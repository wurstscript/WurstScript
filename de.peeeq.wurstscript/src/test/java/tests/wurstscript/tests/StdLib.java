...
/** Helper class to download the standard library, which is required by some test cases */
public class StdLib {

  /** the repo to download */
  private static final String gitRepo = "https://github.com/wurstscript/WurstStdlib2";

  /** version to use for the tests */
  private static final String version = "51f3274640197f957c3c53fb8cbe97c4e96ae898";
  ...
  public static synchronized boolean downloadStandardlib() {
    if (isInitialized) {
      return true;
    }
    ...
    try {
      if (!stdLibFolder.exists()) {
        tempFolder.mkdirs();
        try (Git git = Git.cloneRepository().setDirectory(stdLibFolder).setURI(gitRepo).call()) {
      ...
      try (Git git = Git.open(stdLibFolder)) {
        ...
        if (!head.equals(version)) {
          System.out.println(
              "Wrong version '" + head + "', executing git pull to get '" + version + "'");
