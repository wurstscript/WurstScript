 ...
 *
 * <p>the position of the cursor is denoted by a bar "|" in the test cases
 */
public class GetDefinitionTests extends WurstLanguageServerTest {

  @Test
  public void simpleExample1() {
    CompletionTestData testData =
        input(
            "package test",
            "	function int.foo()",
            "	function int.bar()",
  ...
  public void nonexistantModule() {
    CompletionTestData testData = input("package test", "class A", "	use B|lub");
  ...
  private void testGetDef(CompletionTestData testData, List<String> expectedPositions) {
    ...
    Either<List<? extends Location>, List<? extends LocationLink>> result =
        calculateDefinition(testData);
    if (result.isLeft()) {
      List<String> definitions =
          result.getLeft().stream()
              .map(
                  (Location l) ->
                      l.getRange().getStart().getLine()
                          + ":"
                          + l.getRange().getStart().getCharacter()
                          + "-"
                          + l.getRange().getEnd().getLine()
                          + ":"
                          + l.getRange().getEnd().getCharacter())
              .sorted()
      ...
      List<String> definitions =
          result.getRight().stream()
              .map(
                  (LocationLink l) ->
                      l.getTargetRange().getStart().getLine()
                          + ":"
                          + l.getTargetRange().getStart().getCharacter()
                          + "-"
                          + l.getTargetRange().getEnd().getLine()
                          + ":"
                          + l.getTargetRange().getEnd().getCharacter())
              .sorted()
              .collect(Collectors.toList());
  ...
  private Either<List<? extends Location>, List<? extends LocationLink>> calculateDefinition(
      CompletionTestData testData) {
