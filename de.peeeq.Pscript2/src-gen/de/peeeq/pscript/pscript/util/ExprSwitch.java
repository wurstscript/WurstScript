...
public abstract class ExprSwitch <T> {
	...
	abstract public T caseExprThis(ExprThis exprThis);
	...
	abstract public T caseExprNewObject(ExprNewObject exprNewObject);
...
if ( expr == null) throw new IllegalArgumentException("Switch element must not be null.");
		...
		if (expr instanceof ExprThis) return caseExprThis((ExprThis)expr);
		...
		if (expr instanceof ExprNewObject) return caseExprNewObject((ExprNewObject)expr);
