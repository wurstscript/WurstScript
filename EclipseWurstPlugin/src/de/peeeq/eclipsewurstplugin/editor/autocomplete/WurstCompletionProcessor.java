...
public class WurstCompletionProcessor implements IContentAssistProcessor {
    ...
    // lifecycle makes sure that elements are not null when accessed
    ...
    private ICompletionProposal @Nullable [] calculateCompletion(ITextViewer viewer, final int offset, CompilationUnit cu) {
            ...
            if (elem instanceof ExprEmpty) {
                if (elem.getParent() instanceof Arguments) {
                    ...
                    if (grandParent instanceof ExprFunctionCall) {
                        ...
                        getCompletionsForExistingConstructorCall(offset, completions, c);
                    }
    ...
    private void getCompletionsForExistingConstructorCall(final int offset, List<WurstCompletion> completions, ExprNewObject c) {
        ...
        if (constructorDef != null) {
            ...
            completions.add(makeConstructorCompletion(classDef, constructorDef).withDisableAction(offset));
        }
    }

    private void getCompletionsForExistingMemberCall(final int offset, List<WurstCompletion> completions, ExprMemberMethod c) {
        FunctionDefinition funcDef = c.attrFuncDef();
        if (funcDef != null) {
            completions.add(makeFunctionCompletion(funcDef).withDisableAction(offset));
        }
    }

    private void getCompletionsForExistingCall(final int offset, List<WurstCompletion> completions, ExprFunctionCall c) {
        FunctionDefinition funcDef = c.attrFuncDef();
        if (funcDef != null) {
    ...
    private WurstCompletion makeNameDefCompletion(NameDef n) {
        ...
        IContextInformation contextInformation =
                new ContextInformation(n.getName(), Utils.printElement(n) + " : " + n.attrTyp().getFullName() + " -  defined in " + nearestScopeName(n)); //$NON-NLS-1$

        String additionalProposalInfo2 = n.descriptionHtml();
        double rating = calculateRating(n.getName(), n.attrTyp());
        return new WurstCompletion(replacementString, replacementOffset, replacementLength, cursorPosition, image, displayString, contextInformation,
                additionalProposalInfo2, rating);
    }
