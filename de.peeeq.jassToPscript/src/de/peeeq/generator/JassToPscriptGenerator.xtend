...
class JassToPscriptGenerator implements IGenerator {
	...
	def compile(Resource resource)  
		...
		package jass.natives {
			
			// standard types
			native type Int = integer
			native type Real = real
			native type String = string
			native type Bool = boolean
			native type Code = code
			native type Handle = handle
	...
	def dispatch compile(TypeDef t) '''native type «t.name.compileType» = «t.name» extends «t.extendName.compileType»''' 
	...
	function «f.name.toFirstLower» («f.params.compile») {
		buildin «f.name»(«f.params.compileNames»)
	}
	«ELSE»
	function «f.name.toFirstLower» («f.params.compile»):«f.returnType.name.compileType» {
		return buildin «f.name»(«f.params.compileNames»)		
	}
	«ENDIF»'''
